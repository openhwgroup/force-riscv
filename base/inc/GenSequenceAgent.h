...
namespace Force {
  ...
  */
  ...
  public:
    GenSequenceAgent() : GenAgent(), mpSequenceRequest(nullptr), mBntLevel(0), mSpeculativeBntLevel(0){ } //!< Constructor.
    ASSIGNMENT_OPERATOR_ABSENT(GenSequenceAgent);

    Object* Clone() const override;  //!< Return a cloned GenSequenceAgent object of the same type and content.
    const char* Type() const override { return "GenSequenceAgent"; } //!< Return type of the GenSequenceAgent object.
  ...
  protected:
    GenSequenceAgent(const GenSequenceAgent& rOther) : GenAgent(rOther), mpSequenceRequest(nullptr), mBntLevel(0), mSpeculativeBntLevel(0) { } //!< Copy constructor, do not copy the request pointer.
    ...
    virtual void EndOfTest() {} //!< Handle EndOfTest sequenceRequest.
