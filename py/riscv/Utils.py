...
class LoadGPR64(Sequence):
    ...
    ## Generate instructions to load a 64-bit value into a general purpose register. Reserve the
    # register temporarily if it has not already been reserved.
    #
    ...
    ## Generate instructions to load a 64-bit value into a general purpose register. This method
    # first computes the parameter values required to load the lower bits, if necessary, then
    # generates instructions to load the top 32 bits. After that, it uses the parameter values
    # initially computed to generate the instructions to load the remaining bits. The maximum number
    # of instructions this method generates should be 8. The method attempts use larger shift
    # amounts and avoids adding 0 to reduce the number of instructions required to load values with
    # few set bits.
    #
    #  @param aValue A 64-bit value.
    def _loadValue(self, aValue):
        ...
        for (shift_amount, imm_value) in reversed(imm_params):
            ...
            if imm_value != 0:
                self.genInstruction('ADDI##RISCV', {'rd': self._mRegisterIndex, 'rs1': self._mRegisterIndex, 'simm12': imm_value})

        # Due to limitations in the RISC-V architecture, _load32BitValue() is forced to use
        # instructions that sign-extend the result to 64 bits. This yeilds the wrong value when the
        # argument passed to _load32BitValue() has Bit 31 as the most significant bit. We remove the
        # sign extension by shifting.
        if (value.bit_length() == 32) and (aValue.bit_length() != 64):
            shift_amount = 64 - aValue.bit_length()
            self.genInstruction('SLLI#RV64I#RISCV', {'rd': self._mRegisterIndex, 'rs1': self._mRegisterIndex, 'shamt': shift_amount})
            self.genInstruction('SRLI#RV64I#RISCV', {'rd': self._mRegisterIndex, 'rs1': self._mRegisterIndex, 'shamt': shift_amount})

        self.writeRegister('x%d' % self._mRegisterIndex, aValue, '', True)

    ## Generate instructions to load a value with the top 53 bits set into a general purpose
    # register. This method takes advantage of the sign extension for LUI and ADDI to perform the
    # operation in two instructions.
    #
    ...
    ## Generate instructions to load a 32-bit value into a general purpose register. If Bit 31 is
    # set in loadValue, the value loaded into the register will be sign-extended to 64 bits.
    #
...
class LoopControl(LoopControlBase):
    ...
    def generateLoopBackInstructions(self):
        self.genInstruction('ADDI##RISCV', {'rd': self.mLoopRegIndex, 'rs1': self.mLoopRegIndex, 'simm12': 0xFFF})

        # Try using a conditional PC-relative branch
        branch_pc = self.getPEstate('PC')
        (branch_offset, is_valid, num_hw) = self.getBranchOffset(branch_pc, self.mLoopBackAddress, 12, 1)

        self.notice('Loop Register = x%d, Loop Back Address = 0x%x, PC = 0x%x, Branch Offset = 0x%x, Offset Valid = %d' % (self.mLoopRegIndex, self.mLoopBackAddress, branch_pc, branch_offset, is_valid))

        if is_valid:
            self.mPostLoopAddress = branch_pc + 4
            self.genInstruction('BNE##RISCV', {'rs1': self.mLoopRegIndex, 'rs2': 0, 'simm12': branch_offset, 'NoBnt': 1, 'NoRestriction': 1})
        else:
            # Try using an unconditional PC-relative branch
            (branch_offset, is_valid, num_hw) = self.getBranchOffset((branch_pc + 4), self.mLoopBackAddress, 20, 1)

            if is_valid:
                self.mPostLoopAddress = branch_pc + 4 * 2
                self.genInstruction('BEQ##RISCV', {'rs1': self.mLoopRegIndex, 'rs2': 0, 'simm12': 4, 'NoBnt': 1, 'NoRestriction': 1})
