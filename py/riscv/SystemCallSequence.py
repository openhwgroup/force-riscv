...
class SystemCallSequence(Sequence):
    ...
    def _switchPrivilegeLevel(self, aParams):
        ...
        self._assignRegisters(except_request_results['DataBlockAddrRegIndex'], except_request_results['ActionCodeRegIndex'])

        load_gpr64_seq = LoadGPR64(self.genThread)
        load_gpr64_seq.load(self._mDataBlockAddrRegIndex, except_request_results['DataBlockAddr'])

        self._genPrivilegeLevelSwitchInstructions(except_request_results['InstrSeqCode'], except_request_results['PrivilegeLevel'], except_request_results['TargetAddr'], except_request_results['IntermediateRetAddr'])
    ...
    def _processReturnCode(self, aRetCode, aFunction):
        if aRetCode == 0:
            ...
            self.notice('SystemCallSequence request %s could not be processed successfully; return code %d' % (aFunction, aRetCode))

        (no_skip, valid) = self.genThread.getOption('NoSkip')
        if valid and (no_skip == 1) and (aRetCode != 0):
            self.error('Unable to process SystemCallSequence request and NoSkip option was specified')
    ...
    def _genPrivilegeLevelSwitchInstructions(self, aInstrSeqCode, aTargetPrivLevel, aTargetAddr, aIntermediateRetAddr):
        if aInstrSeqCode == 0:
            # Do nothing
            pass
    ...
    def _genXRet(self, aTargetPrivLevel, aTargetAddr):
        ...
        if priv_level == 1:
            ...
            self.error('Unexpected request to execute xRET instruction to transition from privilege level %s to privilege level %s' % (priv_level, aTargetPrivLevel))
