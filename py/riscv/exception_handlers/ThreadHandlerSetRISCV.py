...
class ThreadHandlerSetRISCV(ThreadHandlerSet):
    ...
    def getScratchRegisterSets(self):
        ...
        handler = self.priv_level_handler_sets[PrivilegeLevelRISCV.M].getSynchronousExceptionHandler(SecurityStateRISCV.DEFAULT, ExceptionClassRISCV.ENV_CALL_FROM_U_MODE)
    ...
    def getVectorBaseAddressSets(self):
        ...
        for ((priv_level, security_state), vector_offset_table) in self.vector_offset_tables.items():
            ...
            vector_base_addr_sets['%s%sVector' % (security_state.name.capitalize(), priv_level_str)] = vector_offset_table
    ...
    def setPrivilegeLevel(self, newSecurityState):
        # The handlers are generated with paging disabled to avoid the cost of initializing paging
        # for tests that do not require it. We also want to minimize the number of execution states
        # that we utilize in order to avoid initializing any more state than necessary. This leads
    ...
    def genRelativeBranch(self, br_target_address):
        (br_offset, is_valid, num_hw) = self.getBranchOffset(self.getPEstate('PC'), br_target_address, 20, 1)
