...
## This class maintains a stack for the saving and restoring of register values by exception
# dispatchers and exception handlers. The stack grows downward in memory such that the top of the
# stack will always have the lowest address of any entry. The stack assumes the conventional RISCV
# link register x1 is used. This register is implicitly saved when a stack frame is created and
# reloaded when a stack frame is freed.

# TODO(Noah): Provide a mechanism for using alternate link registers when there is time to do so if
...
class ExceptionHandlerStackRISCV(Sequence):
    ...
    def generate(self, **kwargs):
        ...
        if self._mStackMemAddr is None:
            self._mStackMemAddr = self.virtualMemoryRequest('PhysicalRegion', {'RegionType': 'ExceptionStack', 'Size': 0x400, 'Align': 8, 'Type': 'D'})

        self._mStackTop = kwargs.get('stack_top')
        if self._mStackTop is None:
            self._mStackTop = self._mStackMemAddr + 0x400
    ...
    ## Remove the top stack frame and restore captured registers with their original values.
    ...
    #
    #  @param aRegIndex The index of the register to read the argument value into.
    #  @param aArgIndex The index of the argument to read. The value should be from 0 to 1 less than
    #       the stack frame size. For example, if the stack frame was created with three registers
    #       r1, r2 and r3, then arg(0) retrieves the value of r1, arg(1) the value of r2 and arg(2)
    #       the value of r3.
    ...
    #
    #  @param aRegIndex The index of the register containing the new argument value.
    #  @param aArgIndex The index of the argument to modify. The value should be from 0 to 1 less
    #       than the stack frame size. For example, if the stack frame was created with three
    #       registers r1, r2 and r3, then arg(0) modifies the value of r1, arg(1) the value of r2
    #       and arg(2) the value of r3.
    ...
    #
    #  @param aRegIndex The index of the register in which to load the read value.
    ...
    #
    #  @param aRegIndex The index of the register containing the value to insert into the stack.
    ...
    ## Initialize the stack pointer register to point to the top of the stack and reserve the
    # register.
    #
    #  @param aLoadStackPointer Flag indicating whether to generate instructions to load the stack
    #       top value into the stack pointer. This is necessary when the stack pointer register has
