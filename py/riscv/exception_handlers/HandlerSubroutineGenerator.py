...
class HandlerSubroutineGeneratorRISCV(ReusableSequence):
    ...
    def generateTableWalkSV32(self, **kwargs):
        try:
            ...
            self.error('INTERNAL ERROR: one or more arguments to generateTableWalk() method missing.')
        self._assignScratchRegisterIndices(handler_context)
        self._genTableWalk( self.LEVELS[32], self.LEVEL_BITS[32], self.LEVEL_MASK[32], self.PTESIZE_SHIFT[32],
                            self.PTE_PTR_VAL, self.PTE_XWRV_MASK, self.PTE_SHIFT, self.PPN_SHIFT )
    ...
    def generateTableWalkSV48(self, **kwargs):
        try:
            ...
            self.error('INTERNAL ERROR: one or more arguments to generateTableWalk() method missing.')
        self._assignScratchRegisterIndices(handler_context)
        self._genTableWalk( self.LEVELS[48], self.LEVEL_BITS[48], self.LEVEL_MASK[48], self.PTESIZE_SHIFT[48],
                            self.PTE_PTR_VAL, self.PTE_XWRV_MASK, self.PTE_SHIFT, self.PPN_SHIFT )
    ...
    #  @param aPteShift  - 2**pte-shift equals the size of page table entry in bytes
    ...
    def _genTableWalk(self, aNumLevels, aLevelBits, aLevelMask, aPteSizeShift, aPtePtrVal, aPteXWRVMask, aPteShift, aPpnShift):
        ...
        for cur_level in range(start_level, -1, -1):
            ...
            self._genLevelWalk(cur_level, aLevelBits, aLevelMask, aPteSizeShift, aPtePtrVal, aPteXWRVMask, aPteShift, aPpnShift)
    ...
    def _genLevelWalk(self, aCurLevel, aLevelBits, aLevelMask, aPteSizeShift, aPtePtrVal, aPteXWRVMask, aPteShift, aPpnShift):
    ...
    def genLoadMemory(self, aDestRegIndex, aAddrRegIndex, aImmOffsetVal):
        if self.getGlobalState('AppRegisterWidth') == 32:
            self.genInstruction('LW##RISCV', {'rd': aDestRegIndex, 'rs1': aAddrRegIndex, 'simm12':aImmOffsetVal, 'NoRestriction': 1})
        else:
            self.genInstruction('LD##RISCV', {'rd': aDestRegIndex, 'rs1': aAddrRegIndex, 'simm12':aImmOffsetVal, 'NoRestriction': 1})
