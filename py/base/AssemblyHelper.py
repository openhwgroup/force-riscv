...
# This class facilitates generating common instructions using common
...
class AssemblyHelper(ABC):
    ...
    # Generate an instruction to load the specified register with the value in
    ...
    #  @param aMinuendRegIndex The index of the register containing the
    ...
    # Associate the specified label with the current PC value. Verify that
    # any previously-recorded branches to this label were correctly targeting
    # the current PC value. Fail if any of the branch targets to do not match.
    # Also, fail if no branches have targeted this label.
    #
    #  @param aLabel The label string.
    def addLabel(self, aLabel):
        if aLabel not in self._mBranchRecords:
            self.mSequence.error(
                "INTERNAL ERROR (%s) BRANCH TARGET (%s) NOT RECORDED"
                % (self._mLabelRoutineName, aLabel)
            )
        ...
        for (branch_pc, branch_offset) in self._mBranchRecords[aLabel]:
            ...
            if target_addr != cur_pc:
                ...
                self.mSequence.notice(
                    "INTERNAL ERROR (%s) BRANCH TARGET (%s) MISMATCH:"
                    % (self._mLabelRoutineName, aLabel)
                )
                self.mSequence.notice(
                    "INTERNAL ERROR (%s) BRANCH ADDRESS: 0x%x, BRANCH OFFSET "
                    "(# OF INSTRS): %d, LABEL OFFSET: %d"
                    % (
                        self._mLabelRoutineName,
                        branch_pc,
                        branch_offset,
                        label_offset,
                    )
    ...
    # Output a debug symbol to the log. Debug symbols can be used to aid
    # understanding of the instruction flow.
    #
    #  @param aSymbol The name of the symbol.
    def logDebugSymbol(self, aSymbol):
        self.mSequence.debug(
            '__SYMBOL__ %016x "%s" %s'
            % (
                self.mSequence.getPEstate("PC"),
                aSymbol,
    ...
    #
    #  @param aTargetAddr The target address of the branch.
    #  @param aImmFieldWidth The number of bits available to hold the branch
    #       offset immediate value.
    def getBranchOffset(self, aTargetAddr, aImmFieldWidth):
        cur_pc = self.mSequence.getPEstate("PC")
        (br_offset, valid, num_hw) = self.mSequence.getBranchOffset(
            br_addr=cur_pc,
            target_addr=aTargetAddr,
            offset_size=aImmFieldWidth,
    ...
    # Record the address, label and offset of a branch instruction. When the
    # label is later passed to the addLabel() method, it will be verified that
    # the branch correctly targets the labeled address.
    #
    #  @param aLabel The label the branch is targeting.
    #  @param aBrOffset The branch offset.
