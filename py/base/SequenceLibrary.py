...
class SequenceLibrary(object):
    ...
    def chooseOne (self):
        ...
        if name is not None:
            ...
            if myClass is not None:
                if issubclass (myClass, Sequence) == True:
                    key = name + path
                ...
                elif issubclass (myClass, SequenceLibrary) == True:
    ...
    def getPermutated (self, skip_weight_check=False):       
        ...
        for i, item in enumerate(self.seqList):
            if skip_weight_check == True or self.seqList[i][3] > 0:
    ...
    # generate sequence library instnace from given sequence library class reference
