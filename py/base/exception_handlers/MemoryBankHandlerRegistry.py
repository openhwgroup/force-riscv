...
#
import operator
from base.exception_handlers.ExceptionHandlerRegistry import ExceptionHandlerRegistry

## This class holds handler sets for each memory bank. It is intended for there to be only one
...
## This class represents a collection of the available handlers in a particular memory bank.
class MemoryBankHandlerRegistry(object):
    ...
    def requestExceptionHandler(self, aExceptionHandlerModuleName, aExceptionHandlerClassName, aFactory, aStack):
        return self._mHandlerRegistry.requestExceptionHandler(aExceptionHandlerModuleName, aExceptionHandlerClassName, aFactory, aStack)
    ...
    #       the generation of exception handlers.
    #  @param aStack The exception handler stack.
    def registerExceptionHandlerWithClassName(self, aExceptionHandlerModuleName, aExceptionHandlerClassName, aFactory, aStack):
        self._mHandlerRegistry.createExceptionHandler(aExceptionHandlerModuleName, aExceptionHandlerClassName, aFactory, aStack)

    ## Return the address boundaries of exception handlers sorted by their starting addresses in
    # ascending order.
