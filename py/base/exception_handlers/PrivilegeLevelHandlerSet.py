...
class PrivilegeLevelHandlerSet(Sequence):
    ...
    def __init__(self, gen_thread, privLevel, memBankHandlerRegistryRepo, factory, exceptionsStack):
        ...
        for security_state in self.getSecurityStates():
            self.security_state_handler_sets[security_state] = self.factory.createSecurityStateHandlerSet(security_state, memBankHandlerRegistryRepo, exceptionsStack)
    ...
    def generate(self, **kwargs):
        ...
        self.default_set_name = kwargs['default_set_name']  # impacts scratch registers, handler generation
    ...
    def assignSynchronousExceptionHandler(self, aAssignmentRequest):
        for security_state in aAssignmentRequest.mSecurityStates:
            self._debugPrint("[PrivilegeLevelHandlerSet:assignSynchronousExceptionHandler at %s] %s --> 0x%x" % (self.privLevel, aAssignmentRequest.mExcClass, aAssignmentRequest.mExcClass.value))

            self.security_state_handler_sets[security_state].assignSynchronousExceptionHandler(aAssignmentRequest)
    ...
    def generateSynchronousHandlers(self, aSecurityState, aSyncDispatcher):
        ...
        handler_assignments = security_state_handler_set.getSynchronousExceptionHandlerAssignments()
        self._generateSynchronousDispatchLevel(self.getExceptionCodeClass(), handler_assignments, aSecurityState, aSyncDispatcher)
    ...
    def generateAsynchronousHandler(self, security_state):
        ...
        handler.generateRoutine(self._getHandlerRoutineName(handler), handler_context=self.createExceptionHandlerContext(err_code, default_mem_bank))
    ...
    def generateUserSyncDispatch(self, security_state, sync_dispatcher):
        ...
        err_code = self.getDispatchErrorCode()
        sync_dispatch_addr = sync_dispatcher.generate(handler_context=self.createExceptionHandlerContext(err_code, default_mem_bank))
    ...
    def genJumpToAsynchronousHandler(self, aSecurityState):
        mem_bank_handler_registry = self.memBankHandlerRegistryRepo.getMemoryBankHandlerRegistry(aSecurityState.getDefaultMemoryBank())
        handler = mem_bank_handler_registry.getExceptionHandler(self.master_async_handler_name)
    ...
    def getSynchronousExceptionHandler(self, security_state, exception_class):
        ...
        handler_assignment = security_state_handler_set.getSynchronousExceptionHandlerAssignment(exception_class)
        mem_bank_handler_registry = self.memBankHandlerRegistryRepo.getMemoryBankHandlerRegistry(handler_assignment.mMemBank)
        handler = mem_bank_handler_registry.getExceptionHandler(handler_assignment.mHandlerClassName)
        return handler
    ...
    def _generateSynchronousDispatchLevel(self, aExceptionCodeClass, aHandlerAssignments, aSecurityState, aSyncDispatcher):
        err_code = self.getDispatchErrorCode()
        default_mem_bank = aSecurityState.getDefaultMemoryBank()
        aSyncDispatcher.generateDispatch(self.createExceptionHandlerContext(err_code, default_mem_bank), aExceptionCodeClass)
        ...
        for (exception_class, handler_assignment) in sorted_handler_assignments:
            if handler_assignment.hasSubassignments():
                dispatch_addresses[exception_class] = self.getPEstate("PC")
                self._generateSynchronousDispatchLevel(handler_assignment.getSubexceptionCodeClass(), handler_assignment.getSubassignments(), aSecurityState, aSyncDispatcher)
            else:
                self._generateSynchronousHandler(exception_class, handler_assignment.mHandlerClassName, handler_assignment.mMemBank, aSecurityState)
        ...
        for (exception_class, handler_assignment) in sorted_handler_assignments:
            if handler_assignment.hasSubassignments():
                assembly_helper.genRelativeBranchToAddress(dispatch_addresses[exception_class])
            else:
                self._genJumpToSynchronousHandler(handler_assignment.mHandlerClassName, handler_assignment.mMemBank)
    ...
    #------------------------------------------------------------------------
    def _generateSynchronousHandler(self, aExceptionClass, aHandlerClassName, aMemBank, aSecurityState):
        ...
        mem_bank_handler_registry = self.memBankHandlerRegistryRepo.getMemoryBankHandlerRegistry(aMemBank)
        handler = mem_bank_handler_registry.getExceptionHandler(aHandlerClassName)

        # call custom handler code generator. that code better add in return (or eret if appropriate)
        # if there is not already a generated handler instance that can service this exception,
        ...
        if not handler.hasGeneratedRoutine(handler_routine_name):
            ...
            # The dispatch code and jump tables will be generated in the default memory bank. We
            # generate handlers in this memory bank in a continuous block without needing to adjust
            # the PC value. For handlers that need to be generated in a different memory bank, we
            ...
            handler.generateRoutine(handler_routine_name, handler_context=self.createExceptionHandlerContext(err_code, aMemBank))
            ...
            self.recordSpecificHandlerBoundary(aMemBank, err_code, start_addr, end_addr)

            self.debug("[GENHANDLER] [%s] NEW CUSTOM SYNC HANDLER AT ADDR: 0x%x; err-code: 0x%x" %
                       (self.privLevel, start_addr, err_code))
        else:
            self.debug("[GENHANDLER] [%s] REUSING CUSTOM SYNC HANDLER: err-code: 0x%x" %
                       (self.privLevel, err_code))

        if hasattr(handler, 'use_addr_table') and (handler.use_addr_table == True):
            info_set = {}
