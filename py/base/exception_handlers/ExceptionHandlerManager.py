...
from base.exception_handlers.MemoryBankHandlerRegistry import MemoryBankHandlerRegistryRepository
...
#
#  1. Define choices to allow user to specify how many exception-sets to instance, where maximum number
#     is no larger than the number of threads in play.
#
#  2. Create the pool of exception-set instances.
#
#  3. Each thread then 'maps'* to an exceptions-set instance.
#
#  *note: maps in the sense that the vector base address registers for the thread are initialized to
#         addresses within the exception-set memory.
#-------------------------------------------------------------------------------------------------------
...
class ExceptionHandlerManager(Sequence):
    ...
    def __init__(self, gen_thread, factory):
        ...
        # TODO(Noah) Implement use of a list of ThreadHandlerSets when there is time to do so. Using
        # a list will allow different threads to utilize different exception handlers.
        self.thread_handler_set = factory.createThreadHandlerSet(gen_thread, self.memBankHandlerRegistryRepo, self.exceptions_stack)

        self.address_table = None
        self.spIndex = None

        # Force provides two default exception handler sets: 'Comprehensive' and 'Fast'
    ...
    def createShallowCopy(self, gen_thread):
        ...
        exceptionsManagerCopy.exceptions_stack = exceptionsManagerCopy.factory.createExceptionHandlerStack(gen_thread)
        exceptionsManagerCopy.address_table = None

        return exceptionsManagerCopy
    ...
    def registerAsynchronousExceptionHandler(self, handler_module_name, handler_class_name):
        for mem_bank in self.getMemoryBanks():
            ...
            mem_bank_handler_registry.registerExceptionHandlerWithClassName(handler_module_name, handler_class_name, self.factory, self.exceptions_stack)
    ...
    def registerSynchronousExceptionDispatcher(self, dispatcher):
        if self.fastMode():
            ...
            self.error("Error: Handler set '%s' does not support user-defined dispatch." % self.default_set_name)
    ...
    # TODO(Noah): Implement creation of multiple exception handler sets when there is time to do so;
    ...
    def generate(self, **kwargs):
        ...
        if not self.fastMode():
            self.spIndex = self.exceptions_stack.generate(sp_index=self.spIndex, load_stack_pointer=False)
        ...
        if not ExceptionHandlerManager.cSharedHandlerSetGenerated:
            ...
            self.thread_handler_set.generate(exceptions_stack=self.exceptions_stack,
                                             address_table=self.address_table,
                                             default_set_name=self.default_set_name)

        self.initializeVectorBaseAddressRegisters()

        # after all exception handlers are set, inform back-end the scratch register sets and vbar_sets
        if not self.fastMode():
        ...
        self.exceptionRequest("UpdateHandlerInfo", {"Function": "FastMode", "is_fast_mode": self.fastMode()})
