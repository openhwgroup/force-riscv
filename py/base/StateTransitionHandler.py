...
#  the StateElements provided to the processStateElement() and
# processStateElements() methods. These methods are intended to be overriden by
# subclasses in order to specify exactly how the State is updated.
class StateTransitionHandler(Sequence):
    ...
    # Return randomly-selected indices of GPRs that have no assigned values in
    # the State being transitioned to. These GPRs may be modified freely to
    # assist with the processing of StateElements. This method returns None if
    # there is an insufficient number of GPRs to satisfy the request.
    #
    ...
    # Return all available indices of GPRs that have no assigned values in the
    # State being transitioned to. These GPRs may be modified freely to assist
    # with the processing of StateElements.
    #
    #  @param aExclude A sequence of GPR indices to exclude from selection.
    ...
    # Specify a list of indicies of GPRs that have no assigned values in the
    # State being transitioned to.
    #
    #  @param aArbitraryGprIndices A list of indices of GPRs that have no
    #       assigned values in the State being transitioned to.
    def setArbitraryGprs(self, aArbitraryGprIndices):
        self._mArbitraryGprIndices = aArbitraryGprIndices

    # Initialize a block of memory with the values contained in the specified
    # StateElements. Generate instructions to set a GPR to point to the start
    # of the memory block. The memory block is initialized with ascending
    # addresses corresponding to ascending indices in the StateElement list
    # provided. For StateElements with multiple value elements, the value
    # elements will be arranged such that a single load instruction from the
    # first memory address for that StateElement will load the value elements
    # in the correct order given the current endianness configuration.
    #
    #  @param aMemBlockPtrIndex The index of a GPR that should be set to point
    #       to the memory block.
    ...
    # Initialize a prepared block of memory with the values contained in the
    # specified StateElements. The memory block that begins as the specified
    # starting address is assumed to be large enough to hold all StateElement
    # values.
    #
    #  @param aMemBlockStartAddr The starting virtual address of the memory
    #       block.
    #  @param aStateElems A list of StateElement objects with values that need
    #       to be placed into the memory block.
