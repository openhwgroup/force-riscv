...
#  This test verifies that conditional branches go to the expected address
#  based on the offset operand value.
class MainSequence(Sequence):
    def generate(self, **kargs):
        ...
        for _ in range(10):
            ...
            expected_target_addr = self._getExpectedTargetAddress(
                instr_pc, instr_record["Addressing"]["Offset"][0]
            )
            if target_addr != expected_target_addr:
                self.error(
                    "Unexpected target address: Expected = 0x%x, Actual = 0x%x"
                    % (expected_target_addr, target_addr)
                )

            if self.getPEstate("PC") not in (
    ...
    # Compute the expected target address for a conditional branch
    # instruction given the original PC value and the offset operand value.
    #
    #  @param aOrigPc The PC value of the conditional branch instruction.
    #  @param aOffsetOprValue The offset operand value for the conditional
    #  branch instruction.
    def _getExpectedTargetAddress(self, aOrigPc, aOffsetOprValue):

        expected_target_addr = 0
        self.debug(
            "_getExpectedTargetAddress aOrigPc:{:#x}, "
            "aOffsetOprValue:{:#x}".format(aOrigPc, aOffsetOprValue)
        )

        # process negative/positive 12b simm value | offset is multiple of
        # 2 bytes, so lsh 1 gets the true byte offset
