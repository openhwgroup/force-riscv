...
class ThreadPartitionGlobalInitSeq(GlobalInitSeqRISCV):
    ...
    def setupThreadGroup(self):
        # Allocate one thread into one group randomly; the other threads remain free
        self.partitionThreadGroup('Random', group_num=1, group_size=1)

## This test verifies the random partition strategy. This test assumes at least 8 total threads are
# used.
    ...
    def generate(self, **kargs):
        ...
        if group_id == 0:
            ...
            if len(thread_groups) != 1:
                self.error('Unexpected thread group count; Expected=%d, Actual=%d' % (1, len(thread_groups)))

            thread_constr = ConstraintSet(thread_groups[0][2])
            if thread_constr.size() != 1:
                self.error('Unexpected group thread count; Expected=%d, Actual=%d' % (1, thread_constr.size()))
            ...
            if len(free_threads) != expected_free_thread_count:
                self.error('Unexpected free thread count; Expected=%d, Actual=%d' % (expected_free_thread_count, len(free_threads)))
            ...
            if thread_constr_0.size() != 1:
                self.error('Unexpected group thread count; Expected=%d, Actual=%d' % (1, thread_constr_0.size()))

            thread_constr_1 = ConstraintSet(thread_groups[1][2])
            if thread_constr_1.size() != 2:
                self.error('Unexpected group thread count; Expected=%d, Actual=%d' % (2, thread_constr_1.size()))
            ...
            if thread_constr.size() != 5:
                self.error('Unexpected group thread count; Expected=%d, Actual=%d' % (5, thread_constr.size()))
