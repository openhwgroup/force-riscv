...
## This test verifies the default thread partiion strategy. We expect all threads to be assigned to
# Group 0.
class MainSequence(Sequence):
    ...
    def generate(self, **kargs):
        ...
        if thread_constr.size() != total_thread_count:
            self.error('Unexpected group thread count; Expected=%d, Actual=%d' % (total_thread_count, thread_constr.size()))
        ...
        # The thread count should not be a valid thread ID; e.g. if there are 16 threads, the
        # largest valid thread ID is 15
        group_id = self.getThreadGroupId(total_thread_count)
        if group_id is not None:
            self.error('Unexpected thread group ID for nonexistent thread; Expected=None, Actual=%d' % group_id)
