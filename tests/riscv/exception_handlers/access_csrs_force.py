...
# Test issues read or read/write accesses to random CSRs, based on
# current privilege level...


class MainSequence(Sequence):
    def generate(self, **kargs):
        # generate sequences of random instructions followed by random
        # CSR access...
    ...
    # -----------------------------------------------------------------------
    # select CSR to access based on current privilege level, + random
    # read/write...
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # random selection from CSR tree, indexed by privilege level and
    # access type...
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # generate instructions to read from CSR, and (optionally) write its
    # value back.
    #
    # check read value, write value, and if unexpected exceptions were taken.
    #
    # Will ASSUME that CSR read value can be written back to that same
    # CSR without side effects.
    # -----------------------------------------------------------------------
    ...
    def genAccessCSR(self, csr, writeback=False):
        ...
        self.genInstruction(
            ...
            {"rd": scratch_reg_index, "rs1": 0, "csr": csr_index},
        )
        ...
        if writeback:
            ...
            self.genInstruction(
                ...
                {"rd": 0, "rs1": scratch_reg_index, "csr": csr_index},
            )
