...
## This test verifies that exceptions are triggered as expected and that generation and simulation
# can continue after the exceptions are handled. We reserve a selection of GPRs to test the
# exception dispatcher register save and restore logic. Reserving the registers prevents the random
# instructions generated by this test from modifying the registers. However, exception handlers may
# modify reserved registers, but must restore them before returning.
class MainSequence(Sequence):
    ...
    ## Randomly configure some of the specified exception codes to be handled at a lower privilege
    # level.
    #
    #  @param aExceptCodes A list of exception codes for configuring delegation.
    def _configureExceptionDelegation(self, aExceptCodes):
        ...
        # TODO(Noah): Uncomment the statement below when the medeleg register is usable; it
        # currently has a specified size of 0.
        #self.genInstruction('CSRRW#register#RISCV', {'rd': 0, 'rs1': medeleg_val_reg_index, 'csr': self.getRegisterIndex('medeleg')})

    ## Switch to a random privilege level.
    def _switchToRandomPrivilegeLevel(self):
        self.genInstruction('MRET##RISCV')

    ## Verify the exception count for the specified exception code has increased.
    #
    #  @param aExceptCode The code of the exception that should have been triggered.
