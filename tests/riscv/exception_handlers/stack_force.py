...
## This test verifies the exception handler stack stores and retrieves values as expected.
class MainSequence(Sequence):
    ...
    def generate(self, **kargs):
        ...
        for addr_offset in range(0, stack_mem_size, 8):
            self.initializeMemory(addr=(stack_mem_addr + addr_offset), bank=0, size=8, data=RandomUtils.random64(), is_instr=False, is_virtual=True)

        stack.generate(stack_mem=stack_mem_addr, stack_top=(stack_mem_addr + stack_mem_size))

        self._verifyRandomStackOperations(stack)
        self._verifyRandomStackFrameOperations(stack)

    ## Verify that random push, pop, peek and modify operations yield the expected results.
    #
    ...
    def _verifyRandomStackOperations(self, aStack):
        for _ in range(RandomUtils.random32(50, 100)):
            ...
            if operation == 'push':
                ...
                exception_handlers_test_utils.assertGprHasValue(self, gpr_index, self._mReferenceStack.pop())
            ...
            # the bottom
                ...
                exception_handlers_test_utils.assertGprHasValue(self, gpr_index, self._mReferenceStack[max_stack_index - stack_index])
    ...
    def _verifyRandomStackFrameOperations(self, aStack):
        for _ in range(RandomUtils.random32(10, 20)):
            ...
            if operation == 'newStackFrame':
                # The link register x1 is automatically captured in every stack frame, so we exclude
                # it to avoid capturing it twice in the same frame.
                gpr_indices = self.getRandomGPRs(RandomUtils.random32(1, 5), exclude=('0,1,%d' % aStack.pointerIndex()))
                ...
                for (gpr_index, expected_gpr_val) in reference_stack_frame:
                    exception_handlers_test_utils.assertGprHasValue(self, gpr_index, expected_gpr_val)
            elif operation == 'arg':
                ...
                exception_handlers_test_utils.assertGprHasValue(self, temp_gpr_index, reference_stack_frame[arg_index][1])
    ...
    ## Randomly choose between initializing the specified register with a random value or loading a
    # random value into the register.
    #
    ...
    def _isStackEmpty(self, aStack):
        ...
        exception_handlers_test_utils.assertValidGprValue(self, stack_pointer_index, valid)
        stack_empty = (stack_pointer_val == aStack._mStackTop)
    ...
    def _assertStackFrameSize(self, aStack, aExpectedFrameSize):
        ...
        if stack_frame_size != aExpectedFrameSize:
            self.error('Stack frame size %d is not equal to the expected size %d' % (stack_frame_size, aExpectedFrameSize))
