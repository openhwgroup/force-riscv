...
#
## Randomly initialize and reserve aGprCount randomly-selected GPRs and return a dictionary mapping
# their indices to their values.
#
...
#
#  @param aSequence The calling sequence.
#  @param aOrigGprValues A dictionary mapping GPR indices to their original values.
...
def assertGprHasValue(aSequence, aGprIndex, aExpectedGprVal):
    ...
    if gpr_val != aExpectedGprVal:
        aSequence.error('Value of register x%d did not match the expected value. Expected=0x%x, Actual=0x%x' % (aGprIndex, aExpectedGprVal, gpr_val))
