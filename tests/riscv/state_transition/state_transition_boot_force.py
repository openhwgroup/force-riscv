...
## A test StateTransitionHandler that verifies the boot process can be overridden with a custom
    ...
    def processStateElement(self, aStateElem):
        ...
        if gpr_index in self.mGprIndices:
            if self.getGlobalState("AppRegisterWidth") == 32:
                ...
                instr = 'LD##RISCV'
            self.genInstruction(instr, {'rd': gpr_index, 'rs1': self.mMemBlockPtrIndex, 'simm12': (gpr_index * 8), 'NoRestriction': 1})
...
class MainSequence(Sequence):
    ...
    def generate(self, **kargs):
        ...
        StateTransition.registerStateTransitionHandler(state_trans_handler, EStateTransitionType.Boot, (EStateElementType.GPR,))
