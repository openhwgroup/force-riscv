...
## A test StateTransitionHandler that verifies getArbitraryGprs() returns the expected results.
class StateTransitionHandlerTest(StateTransitionHandler):
    ...
    def processStateElement(self, aStateElem):
        # It's uncertain how many system-reserved registers there are, but the allowance below
        # should be plenty
        max_arbitrary_gpr_count = 32 - len(self.mSpecGprIndices) - 5
        for _ in range(10):
            arbitrary_gprs = self.getArbitraryGprs(RandomUtils.random32(0, max_arbitrary_gpr_count))

            if self.mSpecGprIndices.intersection(arbitrary_gprs):
                self.error('GPR indices %s were incorrectly designed as arbitrary' % self.mSpecGprIndices.intersection(arbitrary_gprs))

        return True


## This test verifies that only GPRs that have not been specified as part of the State are
# designated as arbitrary.
class MainSequence(Sequence):
    ...
    def generate(self, **kargs):
        ...
        StateTransition.registerStateTransitionHandler(state_trans_handler, EStateTransitionType.Explicit, (EStateElementType.GPR,))
