...
class MainSequence(Sequence):
    ...
    ## Test switching between randomly selected privilege levels with randomly selected parameters.
    ...
    def _getState(self):
        ...
        for reg_field_name in ('MIE', 'SIE', 'UIE', 'SUM', 'MXR', 'MPRV'):
            ...
            if not valid:
                self.error('Register field mstatus.%s does not have a valid value' % reg_field_name)
    ...
    def _verifyState(self, aOrigState, aCurrentState, aSysCallParams):
        ...
        if expected_priv_level is not None:
            if aCurrentState['PrivilegeLevel'] != expected_priv_level:
                self.error('Current privilege level does not match the expected value. Expected=%d, Actual=%d' % (expected_priv_level, aCurrentState['PrivilegeLevel']))

        expected_target_addr = aSysCallParams.get('TargetAddr')
        if expected_target_addr is not None:
            if aCurrentState['PC'] != expected_target_addr:
                self.error('Current PC does not match the expected value. Expected=0x%x, Actual=0x%x' % (expected_target_addr, aCurrentState['PC']))
        ...
        if no_iss != 1:
            ...
            expected_interrupt_mask = self._getExpectedRegisterFieldValue(interrupt_field_name, aSysCallParams.get('InterruptMask'), aOrigState)

            # Ignore UIE; it is hardwired to 0 because the N extension is not supported
            if (expected_interrupt_mask is not None) and (aCurrentState['PrivilegeLevel'] != 0):
                if aCurrentState[interrupt_field_name] != expected_interrupt_mask:
                    self.error('Current mstatus.%s does not match the expected value. Expected=0x%x, Actual=0x%x' % (interrupt_field_name, expected_interrupt_mask, aCurrentState[interrupt_field_name]))

            for reg_field_name in ('SUM', 'MXR'):
                expected_field_val = self._getExpectedRegisterFieldValue(reg_field_name, aSysCallParams.get(reg_field_name), aOrigState)

                if expected_field_val is not None:
                    if aCurrentState[reg_field_name] != expected_field_val:
                        self.error('Current mstatus.%s does not match the expected value. Expected=0x%x, Actual=0x%x' % (reg_field_name, expected_field_val, aCurrentState[reg_field_name]))

            expected_mprv_val = self._getExpectedRegisterFieldValue('MPRV', aSysCallParams.get('MPRV'), aOrigState)
            if (expected_mprv_val is not None) and (priv_level_name == 'M'):
                if aCurrentState['MPRV'] != expected_mprv_val:
                    self.error('Current mstatus.MPRV does not match the expected value. Expected=0x%x, Actual=0x%x' % (expected_mprv_val, aCurrentState['MPRV']))
    ...
    ## Determine the expected register field value based on the original register field value and
    # the specified parameter.
    #
    #  @param aRegFieldName Name of the register field.
    #  @param aRegFieldParam Parameter used to specify a value for the register field for the
    #       privilege level switch.
    #  @param aOrigState The register field value prior to the privilege level switch.
