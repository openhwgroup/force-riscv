...
class MainSequence(Sequence):
    """
    This test case uses the genFreePagesRange API to generate multiple
    consecutive virtual pages.

    Self-checking
    1)  If the 1st element of the returned tuple is a boolean 'is_valid'
        indicator. If the value is false, then the test case is failed.
    2)  Using the returned start and stop address range string to
        calculate the total size of the created virtual pages and checking
        that with the expected size based on the pages requested.
    3)  Generated ldst_instructions using the addresses in the virtual
        pages created. If there is no translation support for the returned
        address the generation of the ldst_instrucion will fail to gen.
    ...
    def generate(self, **kwargs):
        ...
        for number_of_pages in range(2, 20):
            ...
            result = self.genFreePagesRange(
                Number=number_of_pages, PageSize=requested_page_sizes_string
            )
