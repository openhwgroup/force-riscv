...
class MainSequence(Sequence):
    ...
    def generate(self, **kargs):
        ...
        for i in range(10):
            ...
            self.notice(">>>>>>>>>>>>>  LS_Target:     {:012X}".format(instruction_info["LSTarget"]))

            # Self-checking
            # The if conditions are othogonal
            if   chosen_instruction in LDST_Byte_instructions:   target_size = 1
            elif chosen_instruction in LDST_Half_instructions:   target_size = 2
            elif chosen_instruction in LDST_Word_instructions:   target_size = 4
            elif chosen_instruction in LDST_Double_instructions:   target_size = 8
            else: self.error("Unexpected instruction chosen:  {}".format(chosen_instruction))

            # Checking - based on the instruction target size, 1, 2, 4 or 8 bytes should be set to not free.
            # Other bytes should be set to free.  Assert on any mismatches.
            ...
            self.notice(">>>>>>>>>>>>>  The address+01 is free:  {}".format(is_address_plus01_free))
            self.notice(">>>>>>>>>>>>>  The address+02 is free:  {}".format(is_address_plus02_free))
            self.notice(">>>>>>>>>>>>>  The address+03 is free:  {}".format(is_address_plus03_free))
            self.notice(">>>>>>>>>>>>>  The address+04 is free:  {}".format(is_address_plus04_free))
            self.notice(">>>>>>>>>>>>>  The address+05 is free:  {}".format(is_address_plus05_free))
            self.notice(">>>>>>>>>>>>>  The address+05 is free:  {}".format(is_address_plus06_free))
            self.notice(">>>>>>>>>>>>>  The address+05 is free:  {}".format(is_address_plus07_free))
            self.notice(">>>>>>>>>>>>>  The address+08 is free:  {}".format(is_address_plus08_free))
            self.notice(">>>>>>>>>>>>>  The address+16 is free:  {}".format(is_address_plus16_free))
