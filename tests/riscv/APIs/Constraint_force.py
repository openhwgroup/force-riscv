...
## This test verifies that basic Constraint module functionality works as expected. No effort is
# made here to thoroughly test the ConstraintSet API, as this is done in the unit tests. The main
# purpose of this test is to verify that there is nothing obviously wrong with the Python wrapper.
class MainSequence(Sequence):
    ...
    def generate(self, **kargs):
        ...
        if constr_a.size() != 101:
            self.error('Constraint %s reported incorrect size; Expected = 101, Actual = %d' % (constr_a, constr_a.size()))
