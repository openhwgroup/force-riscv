...
class MainSequence(Sequence):
    ...
    # Return true if the specified register is a floating point register and
    # has already been partially or fully initialized. For example, this method
    # will return true for D3 if S3 was already initialized.
    #
    ...
    #
    #  @param aRegId The name of the register.
    #  @param aRegMask A mask representing the bits in the initial value
    #       expected to be in the register.
    ...
    def _verifyInitialization(self, aRegId, aRegMask, aInitialData):
        ...
        self.notice(
            ">>>>>>>>>>   Register ID:  {}     Initial data:  {:016X}     "
            "Returned data:  {:016X}".format(aRegId, aInitialData, readValue)
        )
        ...
        if readValue == (aInitialData & aRegMask):
            self.notice(
                ">>>>>>>>>>>>   Register {} initialized to {:016X} was read "
                "correctly.".format(aRegId, readValue)
            )
        else:
            self.error(
                ">>>>>>>>>>>>   Value read from Register ID {} did not match "
                "value written.    Written value: {:016X}     Read value: "
                "{:016X}".format(aRegId, aInitialData, readValue)
            )
