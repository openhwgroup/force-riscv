...
# This test verifies that a 64-bit immediate value can be loaded into a
...
class MainSequence(Sequence):
    ...
    # Load an immediate value into a random general purpose register; then
    # verify the result.
    #
    ...
    def _testLoadGpr(self, aValue):
        ...
        if gpr_val != aValue:
            self.error(
                "Register %s has an unexpected value: Expected = 0x%x; "
                "Actual = 0x%x" % (gpr_name, aValue, gpr_val)
            )
