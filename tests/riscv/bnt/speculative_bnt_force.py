...
class MyBntSequence(BntSequence):
    ...
    def BntSequence_IntegerArithmetic(self):
        ...
        choice_mod.modifyOperandChoices('GPRs', {'x1' : 10000, 'x2' : 0}) #X2 should be stack pointer
        mod_id = choice_mod.commitSet()

        self.setBntHook(**{'Func' : 'BntSequence_IntegerArithmetic'}) # set the bnt sequence for the following branch instructions
        self.genInstruction("JALR##RISCV", {'rs1' : 1, 'BRarget':'0xc0000000-0xc80000000', 'SpeculativeBnt' : True})
...
class MainSequence(Sequence):
    def generate(self, **kargs):
        ...
        choice_mod = ChoicesModifier(self.genThread)
        choice_mod.modifyOperandChoices('GPRs', {'x0' : 10000, 'x2' : 0}) #X2 should be stack pointer
        mod_id = choice_mod.commitSet()

        id_arithmetic = self.setBntHook(Seq = 'MyBntSequence', Func = 'BntSequence_IntegerArithmetic') # set the bnt function for the following branch instructions
        self.genInstruction("BGE##RISCV", {'SpeculativeBnt' : True}) # some conditional branch instruction
        self.genMetaInstruction("ADDI##RISCV", {"simm12" : 0x100}) # instructions on taken path

        # revert the modifier
        choice_mod.revert(mod_id)

        id_l = self.setBntHook(Func = 'BntSequence_L') # set the bnt function for the following branch instructions
