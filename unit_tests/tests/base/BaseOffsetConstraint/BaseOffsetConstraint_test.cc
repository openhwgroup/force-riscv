...
CASE( "BaseOffsetConstraint test large offset" ) {

  SETUP( "setup" )  {
    ConstraintSet base_offset_constr;

    SECTION( "test 64-bit unsigned offset" ) {
      BaseOffsetConstraint bo_constr_builder(0, 64, 0, MAX_UINT64, true);
      bo_constr_builder.GetConstraint(0x30caff94feb0e36b, 8, nullptr, base_offset_constr);
      EXPECT(base_offset_constr.ToSimpleString() == "0x0-0xffffffffffffffff");
    }

    SECTION( "test 64-bit signed offset" ) {
      BaseOffsetConstraint bo_constr_builder(0x8000000000000000, 64, 0, MAX_UINT64, true);
      bo_constr_builder.GetConstraint(0xf3f2733f4a5fd362, 64, nullptr, base_offset_constr);
      EXPECT(base_offset_constr.ToSimpleString() == "0x0-0xffffffffffffffff");
    }

    SECTION( "test offset too large" ) {
      BaseOffsetConstraint invalid_bo_constr_builder(0, 96, 0, MAX_UINT64, true);
      EXPECT_FAIL(invalid_bo_constr_builder.GetConstraint(0x33b982c3c836, 16, nullptr, base_offset_constr), "invalid-parameter-value");
    }

    SECTION( "test offset too larget after scaling" ) {
      BaseOffsetConstraint invalid_bo_constr_builder(0, 48, 32, MAX_UINT64, true);
      EXPECT_FAIL(invalid_bo_constr_builder.GetConstraint(0xa78fd4bec0a8, 32, nullptr, base_offset_constr), "invalid-parameter-value");
    }
  }
},
