...
class RtlCmdLineOptions(object):
    ...
    #                               |           |         |    |                        |
    cOptions = [ CommandLineOption('rtl',       "",        0, {"action":"store_true"}, "- When present, adds rtl simulation to the work flow"),
                 CommandLineOption('rtl.report.skip', False, 0, {"action":"store_true"}, "- When present, prevents a report from being sent to the triage database"),
                 CommandLineOption('rtl.report.name', "master_run", 1, {}, "- When present, overwrites the default report name with the users choice"),
                 CommandLineOption('rtl.report.xml', False, 0, {"action":"store_true"}, "- When present, dumps the report to an xml file rather than uploading it"), 
...
class RtlAppOptions(object):
    ...
    cOptions = [
        #               "option name" 
        #                    |   "default value"
        #                    |         |      "env variable associated if any"
        #                    |         |                  |
        ApplicationOption('regr',    'logs',          'RTL_REGR'),  # This corresponds to OUTPUT_DIR in RTL makefile
        ApplicationOption('bld_cfg', 'target_config', 'RTL_CFG'),   # This corresponds to BUILD_CFG in RTL makefile
        ApplicationOption('exe',     'uvm_simv_opt',  'RTL_EXE'),   # Allow specifying a diffent simulation executable
        ApplicationOption('report_script_path', '/utils/regression/report.py', ''), # Specify the relative location of the reporting script
        ApplicationOption('rtl_metrics_path', '~/force_regr_cycle_count', ''), #file to save cycle count and intruction count
...
def processRtlControlData(aControlData, aAppParameters):
    ...
    # the location of the default wave_fsdb signal output selection file to copy for use with rerun scripts
