...
class RtlExecutor(IssExecutor):
    ...
    def filter_plus_args(self, a_plus_args):
        ...
        for arg in args_list:
            if arg in [
                ...
                # Since we're dealing with other scripts meant for use without
                # Force, we have to peel back a few of their assumptions about
                # directory structure
    ...
    ##
    #   Makes a copy of the waves_fsdb.do file found in a default location
    #   specified in the config file and copies it with a name change into the
    #   current working directory in order to support the rerun command scripts
    #   that have the dump option enabled.
    #
    #   By default the do file may enable a volume of signals that can lead to
    ...
    ##
    #   Provides a meaningful rerun script at the disk location expected in
    #   the gathered report, such as in gathered.xml
    #
    ...
    def output_rerun_script(self):
        ...
        bsub_rerun_script_path = PathUtils.append_path(
            output_directory, RtlDefaults.BSUB_RERUN_CMD_FILENAME
        )
        bsub_rerun_dump_script_path = PathUtils.append_path(
            output_directory, RtlDefaults.BSUB_RERUN_DUMP_CMD_FILENAME
        )

        base_rerun_cmd = self.rtl_cmd.replace("UVM_NONE", "UVM_HIGH")
        base_rerun_dump_cmd = base_rerun_cmd.replace(
