...
class FrunToCtrlExecutor( AppExecutor ):
    ...
    def skip(self):
        if not self.ctrl_item.fruntoctrl.get('run', False):            
            Msg.user( "[FrunToCtrlExecutor::skip] skipping since run is not set to True..." )
            return True
    ...
    def execute( self ):
        ...
        if not PathUtils.check_file( "./_def_frun.py" ):
            Msg.user( "[FrunToCtrlExecutor::skip] skipping since no _def_frun.py found" )
            return True

        my_cmd = self.mFrunToCtrlCmd 
        if my_cmd is None:
            Msg.user( "[FrunToCtrlExecutor::skip] skipping since no path was given" )
            return True
