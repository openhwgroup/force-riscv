...
class TracediffRiscVExecutor( ToolExecutor ):
    ...
    def skip(self):
        #if not self.ctrl_item.tracecmp.get('run', False):            
        #    Msg.user( "[TracediffRiscVExecutor::skip] skipping since run is not set to True..." )
        #    return True
    ...
    def execute( self ):
        ...
        my_result = SysUtils.exec_process( my_cmd, self.log, self.elog, self.ctrl_item.timeout, True )
        my_use_result = None
        ...
        with open(self.log, 'r') as results_file:
          ...
          my_use_result[0] = int(not success) #This inversion is necessary because int 0 means success to the Summary class.  

        with open(self.log, 'a') as out_file:
          if success == False: 
            out_file.write("tracediff_riscv.log fail, look for | symbols or 'Excpt ID 0x2'; " + str(vbar_symbol_count) + " mismatches, and up to " + str(exception_count) + " suspicious exceptions.")
          else:
            out_file.write("tracediff_riscv.log success, only bootcode difference between standalone and interactive as expected.")
