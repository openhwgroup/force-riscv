...
class RunWithReportParms(object):
    ...
    # pass this object to CmdLineParser and the parameters will be populated
    # by its set_parameters method
    # parameters:
    # self.ctrl_file
    # self.test_dir
    # self.force_path
...
class RunWithReport(object):
    ...
    def _getGatherCommand(self):
        ...
        # TODO use this one instead when confident the gather flow is working
        #  correctly gather_cmd = self._mDirName + "/../misc/gather_force.pl
        #  -u top %s/top_regress_sim.rpt -q" % self._mTestDir
        return gather_cmd
...
class CommandLineParameters(object):
    ...
    parameters = [
        ...
        # |      |                |   |
        # |      |                |   |
        ["-f", "--ctrl-file", 1, {}, "specify control file"],
        ["-D", "--test-dir", 1, {}, "test directory"],
        ["-F", "--force-path", 1, {}, "force path"],
        ["-L", "--label", 1, {}, "label to use"],
        [
            "-M",
            "--master-only",
            0,
        ...
        [
            ...
            1,
            {},
            "directory to generate report from, only effective when using "
            "togather with --report-only or --summary-only",
        ],
