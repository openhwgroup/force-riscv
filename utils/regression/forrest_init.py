...
## 
# Purpose: used to configure an argparse argument parser instantiated in module_run a superclass to forrest_run
# Caveats: setting defaults still requires the user to change the defaults in the Defaults class as weall as in this CommandLineParameters class.
#
class CommandLineParameters(object):
    ...
    #required variable for use with argparse, save and pass on remaider (enabling prevents detecting unknown arguments)
    pass_remainder = False

    #required variable for use with argparse, choosing the RawTextHelpFormatter means the endline characters in the descriptions are not ignored.
    ...
    _parameters_general_options = [
        ...
        # |      |                |   |
        # |      |                |   |
        ["-f", "--control-name=", 1, {}, "- (Optional) Control File: Optional to override the default control file name in the specified\ndirectory, if not specified the default \"_def_fctrl.py\" will be used."],
        ["-d", "--control-dir=", 1, {},  "- (Optional) Location the_force_root + \"tests/\"of initial Control File, Relative path to a directory containing the\nInitial Control File, if not specified the relative path will be \"tests/basic\""],
        ["-w", "--workflow=", 1, {}, "- When present, specifies a particular workflow module to use, otherwise uses the default workflow."],
        ["-t", "--test-base=", 1, {}, "- (Optional) Absolute Path to alternate test base directory: Overrides the Force path, Needs\nto be an absolute path"],
        ["-m", "--mode=", 1, {"default":"regress", "choices":["mock", "regress", "perf"]}, "-mode\n\tmock - dry run\n\tregress (default) - actually generate and/or run simulations\n\tperf - performance tests, generate only"],
        ["-o", "--logfile=", 1, {}, "- when present redirects stdout into specified file"],
        ["-l", "--msg-lev=", 1, {"default":"crit+err+warn+info+noinfo"}, "- Set the output level, implemented as a bit mask example if you want warnings and debug pass 20, 4 + 16\n\t\"nomsg\"   [0x0000]- supresses all message\n\t\"crit\"    [0x0001]- Critical Errors Messages\n\t\"err\"     [0x0002]- Non Critical Errors Messages\n\t\"warn\"    [0x0004]- Include Warning Messages\n\t\"info\"    [0x0008]- Run Info Messages\n\t\"dbg\"     [0x0010]- Debug Information\n\t\"user\"    [0x0020]- Special Messages\n\t\"trace\"   [0x0040]- provides call stack trace info\n\t\"noinfo\"  [0x0080]- supresses the info label\nExample: \"err\"+\"warn\"+\"dbg\" will post messages\n\t[ERROR] - Shows Non Critical Error Messages     and\n\t[WARN]  - shows warning messages                and\n\t[DEBUG] - Shows debug messages                  and nothing else\n\n Default: \"crit+err+warn+info+noinfo\"\nTo use the default log level and add levels prepend that level with a (+)\nTo use the default log level and remove levels prepend that level with a (-)\nExample:\n\t[-l +trace-crit+user] will create an output level err+warn+info+noinfo+trace+user"],
        ]
