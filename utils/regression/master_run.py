...
class MasterRun(ModuleRun):
    ...
    def init_app_setup(self):
        if not self.m_app_setup:
            self.m_app_setup = ApplicationsSetup(
                self.mCmdLineParms, sys.argv, self.mConfigArgs, True, True
            )
    ...
    def run(self):
        ...
        if self.mode == "count":
            Msg.info(
                "Total tasks counted in control file tree: "
    ...
    def locate_control_file(self):
        ...
        # the default
        my_fctrl_path = self.option_def(
            CmdLine.Switches[CmdLine.control_name], CtrlItmDefs.fctrl_name
        )
    ...
    def locate_directory(self, arg_cmd_switch, arg_envar, arg_default):
        ...
        Msg.dbg(
            "arg_cmd_switch[%s], arg_envar[%s], arg_default[%s]"
...
def retrieve_config_argument(aArguments):
    ...
    return (
        PathUtils.resolvePath(config_argument.config[0])
        if config_argument.config is not None
        else None
    )
...
def main():
    ...
    try:
        ...
        if not PathUtils.chdir(my_module.output_dir, True):
            Msg.dbg(
                "Unable to change into: "
                + my_module.output_dir
