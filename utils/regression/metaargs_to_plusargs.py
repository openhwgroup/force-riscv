...
class MetaArgsConversion(object):
    ...
    def __init__(self, aRtlRoot, aMakefile):
        ...
        self._mTopSimPath = PathUtils.include_trailing_path_delimiter(self._mRtlRoot) + "verif/top/sim"
    ...
    def filterList(self, aArgsList):
        syntax_to_skip = [ '+UVM_TC_PATH=', '+UVM_TC_NAME=', '+UVM_TEST_DIR=', '+UVM_TC_SEED=', '+tbench_elf64=', '+ns_mem_init=', '+s_mem_init=', '+ntb_random_seed=', '-reportstats', '-assert', 'nopostproc', '+REGR=', '+REGR_ID=' ]
        filtered_list = [ ]
    ...
    def compareMetaAndPlus(self, aMetaArgs, aPlusArgs):
        ...
        comp_result = "Only in set1: %s\nOnly in set2: %s" % (' '.join(only_in_set1), ' '.join(only_in_set2))
        return comp_result
...
def convert_meta_args(aConversionParms):
    if aConversionParms.meta_args is None:
        if (aConversionParms.plusargs is not None) and len(aConversionParms.cmp_plusargs):
            pass
        else:
    ...
    make_file_path = PathUtils.include_trailing_path_delimiter(script_dir) + "applications/rtl/MetaArgs.make"
    ...
    if aConversionParms.net:
        ...
        if (aConversionParms.plusargs is not None) and len(aConversionParms.plusargs) > 0:
            conversion_result = meta_args_conv.comparePlusAndPlus(aConversionParms.plusargs, aConversionParms.cmp_plusargs)
        else:
            conversion_result = meta_args_conv.compareMetaAndPlus(meta_args, aConversionParms.cmp_plusargs)
    else:
...
""" % sys.argv[0]
    ...
    parameters = [
        ...
        [ "-n", "--net",          0,  { "action":"store_true" },
          "convert meta args to net result."
        ],
        [ "-c", "--cmp-plusargs", 1,  { 'default':'' },
