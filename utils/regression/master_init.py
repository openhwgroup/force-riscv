...
class CommandLineParameters(object):
    ...
    _parameters_persisting_yes_override = [
        ...
        [
            ...
            1,
            {"default": "", "metavar": ""},
            '- When present, "all" is specified, does not remove pass files '
            "in the output\n"
            "  directory for that task; if a comma seperated file-extension "
            "string is\n"
            "  specified, then the type of files will be preserved; otherwise "
            "the only two\n"
            "  files for task that successfully complete is the _def_frun.py "
            "and the process\n"
    ...
    _parameters_non_persistent = [
        ...
        [
            ...
            1,
            {"metavar": ""},
            "- When present, sets iss options to pass as a dictionary for a "
            "specific set of\n"
            "  instructions for any test processed in the first control file, "
            "after that it\n"
            "  must be specified or it will revert to the default which is "
            "empty.\n"
            "  Example:\n"
            '  \t--iss= {"path":"bla/bla"} - will override the default '
            "simulator with bla/bla\n"
        ...
        [
            ...
            1,
            {"metavar": ""},
            "- When present, sets the generator options for all tests "
            "processed in the first\n"
            "  control file only, nested control files will need to provided "
            "this\n"
            "  Example:\n"
            '  \t--generator={"--options":"option-tag":"option-value", ...}\n'
            '  \t--generator={"--options":"op1-tag=op1-val op2-tag=op2-val",'
            '"tag = val"::None ...}',
        ],
    ]
    ...
    _parameters_others = [
        ...
        [
            ...
            1,
            {"metavar": ""},
            "- When present, specifies number of days to keep output "
            "directory, once the files\n"
            "  are removed then the utility will exit and not process "
            "further. If a mode was\n"
            "  not specified then the expire will operate on regression "
            "only, otherwise it\n"
            "  will follow the value in mode except when mock is passed in. "
            "In that case\n"
            "  expire will have no effect.\n"
            "  NOTE: archive and clean can be grouped with regress or perf "
            "to clean or archive\n"
            "  the specific directories.\n"
            "  Example:\n"
            "  \t--expire 4 will preserve output that is 4 or less days "
            "old,\n"
        ...
        [
            ...
            "--msg-lev=",
            1,
            {"default": "crit+err+warn+" "info+noinfo", "metavar": ""},
            "- When present, Updates or replaces the default output level, "
            "which include:\n"
            ...
            '  \t"noinfo"  [0x0080]- supresses the info label, Additional '
            "Levels are:\n"
            ...
            "  To turn on a message type use the (+) operator. To turn off a "
            "message type\n"
            "  use the (-) operator.\n"
            "  Examples:\n"
            "  \t-l +dbg-warn - will turn on debug messages and turn off "
            "warning,\n"
            "  \t-l dbg+info  - will display only debug and info messages "
            "with the INFO label,\n"
            "  \t-l all       - will display all message types,\n"
            "  \t-l nomsg     - will display only system level messages and "
            "critical errors",
        ],
        [
            "--client-lev=",
            ...
            1,
            {"metavar": ""},
            "- When present, sets or updates the message level for the client "
            "application,\n"
            "  this output is written to the client log all options are the "
            "same except\n"
            '  \t"crit", "err", "warn", "info" and "noinfo"\n'
            "  are always enabled in the client and cannot be disabled "
            "otherwise everything is\n"
            "  exactly the same as the -l option. If True is passed the "
            "message level is the\n"
            "  same as that of master run respecting the required message "
            "types.\n"
            "  Example:\n"
            "  \t--client-lev all  - will enable all message types.\n"
            "  \t--client-lev True - will enable the master run level in "
            "addition to the\n"
    ...
    group_descriptions = [
        _group_1_description,
        _group_2_description,
        _group_3_description,
        _group_4_description,
        _group_5_description,
    ]
    group_parameters = [
        _parameters_general_options,
        _parameters_persisting_no_override,
    ...
    # the following variable is for compatibility with the single argument
    # constructor in cmdline_utils.py
    parameters = []
...
class Defaults(object):
    ...
    # location of the default control file relative to master run
    fctrl_dir = "../../tests"
...
class CmdLine(object):
    ...
    Switches = [
        ...
        "min-instr=",
        # 7     Minimum number of instructions to need to be simulated
        ...
        "run-launcher=",
        # 19     Type launcher for client processor. lsf or local
        "process-max=",  # 20     Max number of concurrent operations
        "timeout=",
        # 21     Max number of seconds before any single task times out
        "client-lev=",
        # 22     Log Level in the Client, saved in the client log
        "keep=",  # 23
        "no-archive",  # 24
        "max-fails=",  # 25
