...
class CmdLineParser(object):
    """
    In order to get the usage help to have the desired sections, arguments
    need to be added as groups into the parser. This form of the constructor
    ...
    def __init__(
        ...
        for init_parm in init_parameters:
            try:
                ...
                # for example, if the cmd_line_spec has prog attribute, we
                # will pass it along to ArgumentParser __init__
                pass
            else:
                init_dict[init_parm] = init_parm_value

        # Do we want the default add_help behavior, or for compatibility
        # reasons maybe do we want to prevent this?
        if add_help:
        ...
        # We want to be able to set up conceptual parsing groups so that the
        # help screen retains some similarity according to the grouping that
        # used to be presented with the old command line parsing.
        if group_parameters is not None:
            for index, parameters in enumerate(group_parameters):
                self.argument_groups.append(
                    self.parser.add_argument_group(
                        title=group_names[index],
                        description=group_descriptions[index],
                    )
                )
...
#
#  A minimal object to hold the output of the parse_args methods from
#  argparse as attributes
...
#
# Parse and access parsed command line options. Seems to have overlapping
# functionality with ModuleRun, while other files like forrest_run.py depend
...
class CmdLineOpts(object):
    ...
    ##
    # Takes an argument name, looks it up in the parsed options but returns
    # the externally provided default if none found.
