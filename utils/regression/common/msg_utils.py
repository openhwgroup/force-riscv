...
class MsgLevel:
    ...
    def translate( cls, arg_str ):
        ...
        if arg_str.find( "all" ) >= 0 :
            my_lev |= MsgLevel.trace | MsgLevel.user | MsgLevel.dbg | MsgLevel.info | MsgLevel.warn | MsgLevel.err | MsgLevel.crit
    ...
    def lev_as_str( cls, arg_lev ):
        ...
        my_str = ""
        if arg_lev & MsgLevel.trace : my_str += "+trace"
        if arg_lev & MsgLevel.user  : my_str += "+user"
        if arg_lev & MsgLevel.dbg   : my_str += "+dbg"
        if arg_lev & MsgLevel.info  : my_str += "+info"
        if arg_lev & MsgLevel.warn  : my_str += "+warn"
        if arg_lev & MsgLevel.err   : my_str += "+err"
        if arg_lev & MsgLevel.crit  : my_str += "+crit"
...
class Msg:
    ...
    lev = MsgLevel.crit | MsgLevel.err | MsgLevel.warn | MsgLevel.info | MsgLevel.noinfo # TODO: This is a hack to default messaging to master run's specified default logging level. In the future, make the message utility completely independent of master run.
