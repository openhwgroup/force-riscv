...
class CommandLineOption(ApplicationOption):
    ...
    def __init__(self, aName, aDefault, aNumArgs, aAdditionalArgs, aHelpText, aShortOpt=None, aEnvVar=None):
    ...
    # ["--lsf.sla", "--lsf.sla", 1, {"default":"trg-dvClass", "metavar":""}, "- Specify LSF serice class for scheduling polcy"]
    # Which will be passed to command line parameter parsing utilities to handle.
    def getExpandedList(self):
        ret_list = [self._shortOptionString(), self._optionString(), self._mNumArgs]
    ...
    ## Get option variable to use to look up option value from parsed command line result.
    def _optionVar(self):
        return self._mName
...
## Class for option that is controlled by a command line option with optional associated environment variable.
#
class AppCmdLineOption(CommandLineOption):

    def __init__(self, aName, aDefault, aNumArgs, aAdditionalArgs, aHelpText, aShortOpt=None, aEnvVar=None):
        super().__init__(aName, aDefault, aNumArgs, aAdditionalArgs, aHelpText, aShortOpt, aEnvVar)
        self.mAppName = None
...
class ControlItemOption(ApplicationOption):
    ...
    def isSpecified(self, aCmdLine):
        ...
        check_names = [ self._mName + self._mPreferredConnector, self._mOtherName + self._mOtherConnector]
    ...
    ## Append the default option to passed in command line, _mName is the prefered name to use here, long or short.
    #
