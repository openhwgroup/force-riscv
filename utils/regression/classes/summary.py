...
class SummaryItem( object ):
    ...
    def __init__( self, arg_summary ):
        ...
        self.sum_tups = [ ( "GenCmd"     , self.load_gen_info        , SummaryDetail.GenCmd         ),    # Contains a Gen Command Line
                          ...
                          ( "ISSCommand" , self.load_iss_info        , SummaryDetail.IssCmd         ),    # Contains a ISS Command Line
                          ( "ISSResult"  , self.load_iss_result      , SummaryDetail.IssResult      ),    # Contains a ISS Result Line
                          ( "RTLCommand" , self.load_rtl_info        , SummaryDetail.RtlCmd         ),    # Contains a RTL Command Line
                          ( "RTLResult"  , self.load_rtl_result      , SummaryDetail.RtlResult      ),    # Contains a RTL Result Line
    ...
    def report( self ):
        ...
        if self.detail_flags & SummaryDetail.AnyDetail   == SummaryDetail.Nothing    : my_msg = "Process No Found or is Empty"
            ...
            if self.detail_flags & SummaryDetail.GenCmd           == SummaryDetail.GenCmd          : my_msg += "GenCmd, "
            if self.detail_flags & SummaryDetail.GenResult        == SummaryDetail.GenResult       : my_msg += "GenResult, "
            ...
            if self.detail_flags & SummaryDetail.TraceCmpCmd      == SummaryDetail.TraceCmpCmd     : my_msg += "TraceCmpCommand, "
            if self.detail_flags & SummaryDetail.TraceCmpResult   == SummaryDetail.TraceCmpResult  : my_msg += "TraceCmpResult, "
    ...
    def clean_up( self ):
        ...
        if self.passed and not clean_up_rules.shouldKeepAll():
            # list all the files and delete them except for _def_frun.py and the processor log (forrest.log)
...
class  Summary( object ):
    ...
    def is_terminated(self):
        ...
        # if a terminate proc exists then the return from that proc will be returned otherwise it will be False
        # and should not have any effect and other methods of determining termination should be used
        with self.crit_sec:
