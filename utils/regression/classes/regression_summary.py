...
class RegressionSummaryItem(SummaryItem):
    ...
    def commit_generate(self):
        ...
        self.force_result = SysUtils.ifthen(
            SysUtils.success(self.force_retcode),
            "PASS",
    ...
    def commit_trace_cmp(self):
        ...
        # Msg.dbg( "RegressionSummaryItem::commit_generate" )
        self.trace_cmp_result = SysUtils.ifthen(
            SysUtils.success(self.trace_cmp_retcode),
            "PASS",
            SysUtils.ifthen(self.signal_id is None, "FAIL", "INCOMPLETE"),
        )
    ...
    def get_gen_line(self):
        ...
        my_gline = "Generate[%s] - Return Code: %2d, Test Dir: %s/%05d" % (
            self.force_result,
            self.force_retcode,
            self.task_id,
            self.task_index,
    ...
    def get_iss_line(self):
        ...
        my_sline = None
        my_sline = "ISS-Sim[%s] - Return Code: %2d, Test Dir: %s/%05d" % (
            self.iss_result,
            self.iss_retcode,
            self.task_id,
            self.task_index,
    ...
    def get_rtl_line(self):
        my_sline = None
        my_sline = "RTL-Sim[%s] - Return Code: %2d, Test Dir: %s/%05d" % (
            self.rtl_result,
            self.rtl_retcode,
            self.task_id,
            self.task_index,
    ...
    def get_trace_cmp_line(self):
        ...
        my_tline = None
        my_tline = "Compare[%s] - Return Code: %2d, Test Dir: %s/%05d" % (
            self.trace_cmp_result,
            self.trace_cmp_retcode,
            self.task_id,
            self.task_index,
        )
        ...
        Msg.user(
            "Compare[%s] - Return Code: %2d, Test Dir: %s/%05d"
            % (
                self.trace_cmp_result,
                self.trace_cmp_retcode,
                self.task_id,
...
class RegressionSummary(Summary):
    ...
    # Create a summary file to contain the results from every attempted
    # generation and simulation and process each operation and produce
    # summarized totals for quick analysis
    ...
    # iterate through the tasks dictionary and process the iterms associated
    # with each task
    def process_summary_tasks(self, arg_ofile, arg_sum_level):
        ...
        for my_key in self.tasks:
            try:
                arg_ofile.write("\nTask: %s\n" % (my_key))
                task_instr_count, task_cycle_count = self.process_summary_task(
                    arg_ofile, self.tasks[my_key], arg_sum_level
                )
