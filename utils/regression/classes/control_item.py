...
class ControlItem(object):
    ...
    def processAppControlData(self, aAppCfg, aItemDict, aParentItem):
        ...
        if any(map(lambda val: isinstance(val, dict), app_dict.values())):
            raise TypeError(
                "Control Item Data: Nested dictionary not allowed inside %s, Control Item: %s"
                % (app_tag, aItemDict)
            )
    ...
    # this control item datsa is in the form of a dictionary, populate the
    # control item with the extracted values, where the values do not exist
    # use the
    def load_item_values(self, aAppsInfo, aItemDict, aParentItem):
        ...
        self.group = my_options.get(
            CtrlItmKeys.group,
            parent_options.get(CtrlItmKeys.group, CtrlItmDefs.group),
        )
        self.min_instr = my_options.get(
            CtrlItmKeys.min_instr,
            parent_options.get(CtrlItmKeys.min_instr, CtrlItmDefs.min_instr),
        )
        self.max_instr = my_options.get(
            CtrlItmKeys.max_instr,
            parent_options.get(CtrlItmKeys.max_instr, CtrlItmDefs.max_instr),
        )
        self.num_chips = my_options.get(
            CtrlItmKeys.num_chips,
            parent_options.get(CtrlItmKeys.num_chips, CtrlItmDefs.num_chips),
        )
        self.num_cores = my_options.get(
            CtrlItmKeys.num_cores,
            parent_options.get(CtrlItmKeys.num_cores, CtrlItmDefs.num_cores),
        )
        ...
        self.no_sim = my_options.get(
            CtrlItmKeys.no_sim,
            parent_options.get(CtrlItmKeys.no_sim, CtrlItmDefs.no_sim),
        )
        self.suffix = my_options.get(
            CtrlItmKeys.suffix,
            parent_options.get(CtrlItmKeys.suffix, CtrlItmDefs.suffix),
        )
        self.timeout = my_options.get(
            CtrlItmKeys.timeout,
            parent_options.get(CtrlItmKeys.timeout, CtrlItmDefs.timeout),
        )
        self.seed = my_options.get(
            CtrlItmKeys.seed,
            parent_options.get(CtrlItmKeys.seed, CtrlItmDefs.seed),
        )
