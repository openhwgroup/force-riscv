...
class IGroupByNumber(object):
    ...
    def print_test(self, use_standard_template=True):
        ...
        file_name = file_name.replace(
            ".", "+"
        )  # . will be mistaken as module delimitor when the file is imported
        file_name += "_force.py"
        instrs_list = list()
        for instr in self.instructions:
        ...
        with open(file_name, "w") as output_handle:
            if use_standard_template:
                ...
                from shared.basic_test_template import (
                    basic_non_standard_template_str,
                )
    ...
    def __init__(self, grpSize, use_standard_template=True):
        ...
        self.IGenList.append(
            IGroupByNumber(
                ...
                "GenOnly_Grouping_0"
                if use_standard_template
                else "GenOnly_Grouping_NonStandard_0",
            )
        )
    ...
    def add_instruction(self, instr, gen_only, atomic):
        ...
        if gen_only is True:
            ...
            groupingPrefix = (
                "Atomic_Grouping_"
                if self.use_standard_template
                else "Atomic_Grouping_NonStandard_"
            )
        else:
