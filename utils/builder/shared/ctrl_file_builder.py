...
class CtrlFileBuilder():
    ...
    def gen_def_ctrl_file(self):
        ...
        ctrl_item = CtrlItem(False, False, False, self.sArchName, "",
                             "default", [], [], [])
        ...
        for ctrl_item_prefix in self.mCtrlItemPrefixes:
            if ctrl_item_prefix == "atomic":
                atomic_ctrl_item = CtrlItem(True, False, False, self.sArchName,
                                            "atomic/", "atomic", ["options"],
                                            [], [])
                ...
                print("ERROR: def fctrl not processing prefix: {}".format(
                    ctrl_item_prefix))
        ctrl_file_string += "]"
    ...
    def gen_noiss_ctrl_file(self):
        ctrl_file_string = ""
        ctrl_file_string += ctrl_items_template
        ctrl_item = CtrlItem(True, False, False, self.sArchName, "", "default",
                             ["noiss"], [], [])
        ctrl_file_string += ctrl_item.gen_ctrl_item_string()
        ctrl_file_string += ctrl_item_separator
        for ctrl_item_prefix in self.mCtrlItemPrefixes:
            if ctrl_item_prefix == "atomic":
                ...
                genonly_ctrl_item = CtrlItem(True, True, False, self.sArchName,
                                             "genonly/", "genonly", ["noiss"],
                                             ["nosim"], [])
                ctrl_file_string += genonly_ctrl_item.gen_ctrl_item_string()
                ctrl_file_string += ctrl_item_separator
            else:
                print("ERROR: noiss fctrl not processing prefix: {}".format(
                    ctrl_item_prefix))
        ctrl_file_string += "]"
        return ctrl_file_string
    ...
    def gen_perf_ctrl_file(self):
        ctrl_file_string = ""
        ctrl_file_string += ctrl_items_template
        ctrl_item = CtrlItem(False, True, True, self.sArchName, "", "default",
                             [], ["group"], ["group"])
        ctrl_file_string += ctrl_item.gen_ctrl_item_string()
        ctrl_file_string += ctrl_item_separator
        for ctrl_item_prefix in self.mCtrlItemPrefixes:
            if ctrl_item_prefix == "atomic":
                atomic_ctrl_item = CtrlItem(True, True, True, self.sArchName,
                                            "atomic/", "atomic", ["options"],
                                            ["group"], ["group"])
                ctrl_file_string += atomic_ctrl_item.gen_ctrl_item_string()
                ctrl_file_string += ctrl_item_separator
            elif ctrl_item_prefix == "genonly":
                genonly_ctrl_item = CtrlItem(True, True, True, self.sArchName,
                                             "genonly/", "genonly", ["noiss"],
                                             ["nosim", "group"], ["group"])
                ctrl_file_string += genonly_ctrl_item.gen_ctrl_item_string()
                ctrl_file_string += ctrl_item_separator
            else:
                print("ERROR: perf fctrl not processing prefix: {}".format(
                    ctrl_item_prefix))
        ctrl_file_string += "]"
        return ctrl_file_string
...
class CtrlItem():
    ...
    def __init__(self, hasGenerator, hasOptions, hasPerformance, archName,
                 fnamePrefix, groupName, generatorArgs, optionsArgs,
                 performanceArgs):
