...
class G_InstructionAdjustor(InstructionAdjustor):
    def __init__(self):
        ...
        self._mFpSrcDstPattern = re.compile(
            r"^F[A-Z]*.(?P<dst>\w*).(?P<src>\w*)$"
        )
        self._mLdStPattern = re.compile(
            r"^F{0,1}(?P<access>[LS])(?P<size>[A-Z])U{0,1}$"
        )
    ...
    def adjust_instruction_by_format(self, aInstr):
        ...
        if aInstr.name.startswith("AMO"):
            ...
            add_amo_addr_operand(
                aInstr, "rs1", "ReadWrite", atomic_size, "AtomicRW"
            )
        ...
        if aInstr.name.startswith("SC"):
            ...
            add_amo_addr_operand(
                aInstr, "rs1", "Write", atomic_size, "Ordered"
            )
        ...
        if "rs3" in aOpr.name:
            ...
            if aInstr.name.startswith("F"):
                ...
                if ld_st_result is not None:
                    adjust_gpr_operand(
                        ...
                        "Read",
                        "FPR",
                        int_ldst_size(ld_st_result.group("size")),
                    )
                else:
                    adjust_gpr_operand(
                        aOpr, "Read", "FPR", self._FpInstrSize(aInstr.name)
            ...
            if ld_st_result is not None:  # rs1 - int gpr bols addressing opr
                ...
                if src_is_fp:
                    ...
                    adjust_gpr_operand(aOpr, "Read", "GPR", 8)
            elif aInstr.name.startswith("F") and not aInstr.name.startswith(
                "FENCE"
    ...
    def adjust_rd(self, aInstr, aOpr):
        if aInstr.name.startswith("F") and not aInstr.name.startswith("FENCE"):
            ...
            if fp_rd_gpr_result is not None:  # fp instrs with gpr target
                ...
                ld_st_result is not None
            ):  # fp ld instrs - different naming convention (no .<size> which needed for catchall)
                adjust_gpr_operand(
                    aOpr,
                    "Write",
                    "FPR",
...
class V_InstructionAdjustor(InstructionAdjustor):
    ...
    def adjust_instruction_by_format(self, aInstr):
        ...
        if aInstr.iclass == "VectorLoadStoreInstruction":
            ...
            if addr_mode in ["11", "01"]:
                add_vec_indexed_addr_operand(
                    aInstr,
                    "rs1",
                    "vs2",
                    ldst_access(ls_result.group("access")),
                )
                if (
                    aInstr.find_operand("vs3", fail_not_found=False)
                    is not None
                ):
                    add_differ_attribute(aInstr, "vs2", "vs3")
                elif (
                    aInstr.find_operand("vd", fail_not_found=False) is not None
                ):
                    add_differ_attribute(aInstr, "vs2", "vd")
            elif addr_mode in ["10"]:
                add_vec_strided_addr_operand(
                    aInstr,
                    "rs1",
                    "rs2",
                    ldst_access(ls_result.group("access")),
    ...
    def adjust_rs(self, aInstr, aOpr):
        if aOpr.name == "rs1":
            if (
                aInstr.iclass == "VectorLoadStoreInstruction"
                or aInstr.iclass == "VectorAMOInstructionRISCV"
            ):
                ...
                if ".F" in aInstr.name:
                    ...
                    adjust_gpr_operand(
                        aOpr, "Read", self._Rs1Regtype(aInstr), 4
                    )

            if aInstr.name.startswith(
                "VSETVL"
            ):  # set rs1 class for both vsetvl operations
            ...
            if (
                aInstr.name == "VSETVL"
            ):  # set rs2 class for just vsetvl (replaced by zimm in vsetvli)
