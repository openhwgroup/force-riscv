...
def add_reg_branch_operand(aInstr, aOprName, aScale):
    ...
    subop_dict = {"base": aOprName}
    add_addressing_operand(
        aInstr, None, "Branch", "RegisterBranchOperand", subop_dict, attr_dict
    )


def add_cond_branch_operand(
    aInstr, aOprName, aScale, aClass="ConditionalBranchOperandRISCV"
):
    if aClass == "CompressedConditionalBranchOperandRISCV":  # C.BEQZ or C.BNEZ
        theCondition = aInstr.name.replace(
            ".", ""
        )  # remove the "." in the mnemonic
    else:
        theCondition = aInstr.name
    attr_dict = {"offset-scale": aScale, "condition": theCondition}
    subop_dict = {"offset": aOprName}
    add_addressing_operand(
        aInstr, None, "Branch", aClass, subop_dict, attr_dict
    )
...
def add_vec_strided_addr_operand(aInstr, aOprName, aIndexName, aAccess):
    reg_count = (
        int(aInstr.find_operand("custom").regCount)
        if "SEG" in aInstr.name
        else 1
    )
    width = get_element_size(aInstr.find_operand("const_bits"))
...
def add_whole_register_layout_operand(
    aInstr, aRegCount=1, aRegIndexAlignment=1
):
    layout_opr = Operand()
...
def add_layout_operand(aInstr):
    ...
    if aInstr.name in load_store_whole_register:
        ...
        add_whole_register_layout_operand(
            aInstr, aRegCount=reg_count, aRegIndexAlignment=reg_count
        )
        ...
        add_custom_layout_operand(
            aInstr, aRegCount=reg_count, aElemWidth=elem_width
        )
    else:
...
def adjust_register_layout(aInstr):
    ...
    if ".W" in aInstr.name:
        adjust_source = True
    elif aInstr.name.startswith("VSEXT.VF") or aInstr.name.startswith(
        "VZEXT.VF"
    ):
        adjust_source = True
