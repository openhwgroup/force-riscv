...
class C_InstructionAdjustor(InstructionAdjustor):
    ...
    def adjust_instruction_by_format(self, aInstr):
        ...
        for opr in aInstr.operands:
            ...
            if opr.name in ["rs1", "rs1'"]:
                self.adjust_rs1(aInstr, opr)
            elif opr.name in ["rs2", "rs2'"]:
                self.adjust_rs2(aInstr, opr)
            elif opr.name in ["rd", "rd'"]:
                self.adjust_rd(aInstr, opr)
            elif opr.name.startswith("imm"):
                self.adjust_imm(aInstr, opr)
            elif opr.name.startswith("simm"):
                self.adjust_simm(aInstr, opr)
            elif opr.name == "shamt":
                self.adjust_imm(aInstr, opr)
            else:
                pass
        ...
        if aInstr.name in [
            ...
            if len(aInstr.operands) == 3:
                ...
                for opr in tmp:
                    if opr.name == "rd'":
                        aInstr.operands[0] = opr  # loads
                    if opr.name == "rs1'":
                        aInstr.operands[1] = opr  # both
                    if opr.name == "imm5":
                        aInstr.operands[2] = opr  # both
                    if opr.name == "rs2'":
                        aInstr.operands[0] = opr  # stores
            else:
                raise BuilderException(
                    "Instruction {} should have 3 operands, "
        ...
        if aInstr.name in ["C.BEQZ", "C.BNEZ"]:
            ...
            add_cond_branch_operand(
                aInstr, "simm8", 1, "CompressedConditionalBranchOperandRISCV"
            )
        ...
        if self.isLoadStoreOp:
            ...
            if opr.name == "imm5":
                add_bols_addr_operand(
                    aInstr, "rs1'", "imm5", mem_access, self.size, self.scale
                )
            # Stack-pointer loads/stores
            else:
                add_implied_register(aInstr, "x2", "GPR", "Read", 1)
                add_bols_addr_operand(
                    aInstr, "x2", "imm6", mem_access, self.size, self.scale
                )
    ...
    def adjust_rs1(self, aInstr, aOpr):
        ...
        if "rs1" == aOpr.name:
            adjust_gpr_operand(
                aOpr, "Read", "GPR", 0, "Nonzero GPRs"
            )  # the self.size is not used
    ...
    def adjust_rs2(self, aInstr, aOpr):
        ...
        if "rs2" == aOpr.name:
            if aInstr.name in ["C.ADD", "C.MV"]:
                adjust_gpr_operand(
                    aOpr, "Read", self.regType, self.size, "Nonzero GPRs"
                )
        ...
        #    C.SW, C.XOR, C.FSD, C.FSW
            ...
            if self.isFloatOp:
                ...
                adjust_gpr_operand(
                    aOpr, "Read", self.regType, self.size, choices
                )
            else:
                adjust_gpr_operand(
                    aOpr, "Read", self.regType, self.size, "Prime GPRs"
                )
    ...
    def adjust_rd(self, aInstr, aOpr):
        ...
        if "rd" == aOpr.name:
            if self.isFloatOp:
                ...
                adjust_gpr_operand(
                    aOpr, "Write", self.regType, self.size, choices
                )
            elif aInstr.name == "C.LUI":
                adjust_gpr_operand(
                    aOpr, "Write", self.regType, self.size, "GPRs not x0, x2"
                )
            elif aInstr.name in ["C.LI", "C.LWSP", "C.LDSP", "C.MV"]:
                adjust_gpr_operand(
                    aOpr, "Write", self.regType, self.size, "Nonzero GPRs"
                )
            else:
                adjust_gpr_operand(
                    aOpr, "ReadWrite", self.regType, self.size, "Nonzero GPRs"
                )
        ...
        #    C.SRLI, C.SRLI64, C.SUB, C.XOR, C.ADDW, C.SUBW, C.LW, C.ADDI4SPN, C.FLW, C.FLD
            ...
            if self.isFloatOp:
                ...
                adjust_gpr_operand(
                    aOpr, "Write", self.regType, self.size, "Prime GPRs"
                )
            else:
                adjust_gpr_operand(
                    aOpr, "ReadWrite", self.regType, self.size, "Prime GPRs"
                )
