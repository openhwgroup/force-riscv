...
class Files:
    ...
    def __init__(self, aArguments=None):
        ...
        (self.mAppRegisterFile, self.mAppFlag) = self.setupFile(aArguments.app,
                                                                'app')
        ...
        (self.mFieldChoicesFile, _) = self.setupFile(aArguments.field_choices,
                                                     'field_choices')
    ...
    def useModification(self):
        ...
        if len(head_tail) == 1:
            exec(
                'import register_changes.' + self.mRegisterChangeFile +
                ' as data',
                globals())
        else:
            sys.path.insert(0, head_tail[0])
            exec('import register_changes.' + head_tail[1] + ' as data',
                 globals())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--system',
                        help='Path to existing system register XML file')
    parser.add_argument('--app', help='Path to existing app register XML file')
    parser.add_argument('--impl',
                        help='Path to existing implementation defined '
                             'register XML file')
    parser.add_argument('--register_choices',
                        help='Path to existing register choices XML file')
    parser.add_argument('--field_choices',
                        help='Path to existing field choices XML file')
    parser.add_argument('--data',
                        help='Path to modification file for already defined '
                             'registers')
