...
# RiscVRegDef.py
#
# This file defines the RISCV register and its associated register and bit fields and also handles
...
class Register:
    ...
    ## Returns true if skip physical flag is set to anything other than 0, false otherwise
...
class RegisterFile:
            ...
            if register.attrib['boot'] != '0':
                ...
                for field in register:
                    ...
                    if sum_bit_size != int(field.attrib['size']):
                        raise ValueError('Sum of bit sizes (%s bits) and field size (%s bits) for '
                            'field \'%s\' in register \'%s\' are different' % (sum_bit_size,
                            field.attrib['size'], field.attrib['name'], register.attrib['name']))

                if sum_size != int(size):
                    raise ValueError('Sum of field sizes (%s) and register size (%s) for register '
                        '\'%s\' are different' % (sum_size, size, register.attrib['name']))
        ...
        for physical_register in physical_registers:
            if name == physical_register.attrib['name']:
                ...
                if size != physical_register_size:
                    raise ValueError('Register \'%s\' size (%s bits) and physical size (%s bits) '
                        'are different' % (name, size, physical_register_size))
    ...
    def addRegisterToTree(self, aTree, aRegister):
        ...
        if found_register is not None:
            ...
            choice_elements = self.mRegisterChoicesTree.findall('choices') if\
                self.mRegisterChoicesTree is not None else None
            found_choice = None
                    ...
                    if register.attrib['name'] == aRegister['register']:
                        ...
                        for register_field in register:
                            ...
                            if len(register_field.getchildren()) is 0:
                                ...
                                register_field.attrib['size'] =\
                                    str(self.getFieldSizeFromBitsInRegisterField(register_field))
                        ...
                        for register_field in aRegister['fields']:
                            ...
                            for current_field in register:
                                ...
                                if register_field['field'] == current_field.attrib['name']:
                                    ...
                                    if shift_size == current_shift or current_shift_size == shift:
                                        current_field[0].attrib['shift'] =\
                                            str(min(current_shift, shift))
                                        current_field[0].attrib['size'] = str(current_size + size)
                                        current_field.attrib['size'] =\
                                            current_field[0].attrib['size']
                                    else:
                            ...
                            register_field_attributes = {'name':register_field_name,\
                            ...
                            register_field_element = ET.SubElement(register, register_field_tag,
                                register_field_attributes)
                            register_field_element.tail = '\n'
                            bit_field_element = ET.SubElement(register_field_element, 'bit_field',\
                                {'size':str(register_field['size']),\
                                'shift':str(register_field['shift'])})
                            bit_field_element.tail = '\n'

                        register.attrib['size'] = str(new_size)
                        register.attrib['boot'] = str(BootPriority.getBootPriority(\
    ...
    ## Returns the calculated size of a register field by adding up the total sizes of all the bit
    #  fields
    ...
    def addNewRegisterToRegisterChoices(self, aRegister, aReadChoice = True, aWriteChoice = True):
        ...
        if register_type != 'SysReg':
            print('Not adding system register choices for register \'%s\' (type: \'%s\')'
                % (aRegister['register'], register_type))
            return
    ...
    def addNewRegisterToTree(self, aTree, aRegister):
        ...
        register_properties = {'index':index, 'name':name, 'size':'%d' % size, 'boot':'%d' % boot,
            'type':register_type}
        if register_class:
    ...
    ## Deletes specified register and associated register choices from provided tree
    ...
    def addPhysicalRegisterToTree(self, aTree, aRegister):
        ...
        new_physical_register = ET.SubElement(physical_registers, 'physical_register',\
        ...
        for field_choice in field_choices:
            if field_choice.attrib['name'] == aChoice['field_name']:
                ...
                for choice in aChoice['choices']:
                    ...
                    ET.SubElement(field_choice, 'choice', {'value':child['value'],
                        'description':child['description'], 'weight':weight})
                break

        if not update_flag:
            new_field_choice = ET.SubElement(field_choices, 'choices',
                {'name':aChoice['field_name'], 'type':'RegisterFieldValue'})
            for choice in aChoice['choices']:
                weight = choice.get('weight', 10)
            ET.SubElement(new_field_choice, 'choice', {'value':choice['value'],
                'description':choice['description'], 'weight':weight})
    ...
    def sortChildrenByAttribute(self, aParentNode, aAttribute):
        if len(aParentNode.getchildren()) > 0:
            if aAttribute in aParentNode[0].attrib:
                aParentNode[:] = sorted(aParentNode, key = lambda child: self.getProperSortKey(child, aAttribute))
    ## Determines if the supplied attribute is a hex string or something else (like a name)
