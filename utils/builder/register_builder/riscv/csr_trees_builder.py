...
class CSRTreeBuilder:
    ...
    def setupCSRIndices(self):
        ...
        for i in range(0,0xfff):
            # standard user CSRs...
            if   i >= 0     and i < 0xff:  self.mCSRTypesByIndex[i] = ( 'User', 'RW' )
            elif i >= 0x400 and i < 0x4ff: self.mCSRTypesByIndex[i] = ( 'User', 'RW' )
            elif i >= 0xc00 and i < 0xc7f: self.mCSRTypesByIndex[i] = ( 'User', 'R'  )
            elif i >= 0xc80 and i < 0xcbf: self.mCSRTypesByIndex[i] = ( 'User', 'R'  )
            # standard supervisor CSRs...
            if   i >= 0x100 and i < 0x1ff: self.mCSRTypesByIndex[i] = ( 'Supervisor', 'RW' )
            elif i >= 0x500 and i < 0x5bf: self.mCSRTypesByIndex[i] = ( 'Supervisor', 'RW' )
            elif i >= 0x900 and i < 0x9bf: self.mCSRTypesByIndex[i] = ( 'Supervisor', 'RW' )
            elif i >= 0xd00 and i < 0xdbf: self.mCSRTypesByIndex[i] = ( 'Supervisor', 'R'  )
            # standard hypervisor CSRs...
            if   i >= 0x200 and i < 0x2ff: self.mCSRTypesByIndex[i] = ( 'Hypervisor', 'RW' )
            elif i >= 0x600 and i < 0x6bf: self.mCSRTypesByIndex[i] = ( 'Hypervisor', 'RW' )
            elif i >= 0xa00 and i < 0xabf: self.mCSRTypesByIndex[i] = ( 'Hypervisor', 'RW' )
            elif i >= 0xe00 and i < 0xebf: self.mCSRTypesByIndex[i] = ( 'Hypervisor', 'R'  )
            # standard machine CSRs...
            if   i >= 0x300 and i < 0x3ff: self.mCSRTypesByIndex[i] = ( 'Machine', 'RW' )
            elif i >= 0x700 and i < 0x7af: self.mCSRTypesByIndex[i] = ( 'Machine', 'RW' )
            elif i >= 0xb00 and i < 0xbbf: self.mCSRTypesByIndex[i] = ( 'Machine', 'RW' )
            elif i >= 0xf00 and i < 0xfbf: self.mCSRTypesByIndex[i] = ( 'Machine', 'R'  )
            # machine-mode Debug CSRs...
            elif i >= 0x7b0 and i < 0x7bf: self.mCSRTypesByIndex[i] = ( 'Debug', 'RW' )
            # one known custom (machine mode) CSR...
            elif i == 0x7c0: self.mCSRTypesByIndex[i] = ( 'Custom', 'RO' )
            
    ## --------------------------------------------------------------------------------------------
    ## parse system registers starter file to glean all CSR definitions. Group CSRs by
    ## privilege level and access 'rights'...
    ## --------------------------------------------------------------------------------------------
    ...
    def parseSystemRegistersFile(self):
        ...
        for register in registers:
            ...
            try:
                ...
                if mode=='User':
                    self.mUserRW_CSRs.append(reg_name) if access=='RW' else self.mUserRO_CSRs.append(reg_name) 
                if mode=='Supervisor':
                    self.mSupervisorRW_CSRs.append(reg_name) if access=='RW' else self.mSupervisorRO_CSRs.append(reg_name) 
                if mode=='Hypervisor':
                    self.mHypervisorRW_CSRs.append(reg_name) if access=='RW' else self.mHypervisorRO_CSRs.append(reg_name) 
                if mode=='Machine':
                    self.mMachineRW_CSRs.append(reg_name) if access=='RW' else self.mMachineRO_CSRs.append(reg_name) 
    ...
    def deselectCSR(self,csr):
        ...
        # a CSR should be 'deselected' if it appears in one of the two lists above,
        # either by exact match or (in the case of an indexed CSR (example: pmpaddr0) as a sub-string
...
usage_str = """
Use this script to produce a CSR 'trees' file, from the system registers starter file, as follows:

   ./csr_trees_builder.py -i <starter file> -o <trees file>

   or:

   ./csr_trees_builder.py --system_registers_starter_file=<starter file> --csr_trees_file=<trees file>
