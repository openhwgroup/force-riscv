...
def usage():
    usage_str = (
        """Apply patch (diff) file from one build to another.
  -p, --patch specify the name of the patch file.
  -r, --revision specify the revision to check out
  -s, --source if specified apply patch uses the referenced patch as the
        source rather than the svn checkout latest
...
def apply_patch():
    try:
        opts, args = getopt.getopt(
            sys.argv[1:],
            "htz:r:p:s:",
            ["help", "test", "patch=", "revision=", "source_dir="],
        )
    ...
    if test_set:
        # remove .svn directories for test purpose (to compare the merged dir
        # with target dir)
