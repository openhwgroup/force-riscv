...
def setupArguments():
    parser = argparse.ArgumentParser(description='A script to run CppCheck', formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('path', nargs='*', help='Usage: \'FORCE_ROOT/utils/misc/run_cppcheck.py [path to file]\'\nDefaults to ./\nCan also be \'svn\' to run against each added, replaced, or modified file', default=['.'])
    # parser.add_argument('--addon_path', nargs='*', help='path(s) to script(s) to run on Cppcheck dump file')
    parser.add_argument('--xml', nargs='?', help='Displays XML output\nDefaults to \'No\'\nUseful for getting error id (for suppression)', default='No')
    parser.add_argument('--inline', nargs='?', help='Suppresses findings for things marked with \'// cppcheck-suppress\' in source\nDefaults to \'Yes\'', default='Yes')
    return parser
...
def parseSvnStatus():
    ...
    for line in raw_svn_status.split('\n'):
        ...
        if contents[0] == 'A' or contents[0] == 'M' or contents[0] == 'R':
            if not 'unit_tests' in contents[1] and not 'py' in contents[1]:
...
def runCppcheck(args):
    ...
    root = args.path[0]
    paths = [os.path.join(args.path[0], 'base', 'src'), os.path.join(args.path[0], 'riscv', 'src')]

    if args.path[0].casefold() == 'svn'.casefold(): # svn
        # from CppCheck manual: For historical reasons, --enable=style enables warning, performance, portability, and style messages.
        cmd_line.append('--enable=style')
        (root, paths) = parseSvnStatus()
    elif args.path[0].endswith('.cc') or args.path[0].endswith('.h'): # file(s)
        # from CppCheck manual: For historical reasons, --enable=style enables warning, performance, portability, and style messages.
        cmd_line.append('--enable=style')
    ...
    cmd_line.append('--suppress-xml=' + os.path.join(root, 'utils', 'misc', 'cppcheck_suppress.xml'))
