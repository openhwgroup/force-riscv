...
def force_init():
    ...
    # check to see if "tests" path exist if not then something has really gone wrong
    # and recovery from second case is impossible raise a fatal exception
    if not( os.path.exists( my_force_path + "/tests/" )):
        raise OSError( "\"" + my_force_path + "/tests/\" does not exists, check that it exists as a sub directory of Force" )
