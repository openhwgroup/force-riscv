...
class CtrlItmKeys(object):
    ...
    fname = "fname"
    fdir = "control-dir"  # TODO: control file directory, could be implicit,
    # relitive, absolute, or current
...
# this is slated to be a temporary solution, there will be times when we
# process files without threads or just generate the frun control files
class ControlItemActionType:
    ...
    def translate(cls, arg_str):
        if arg_str == "write-only":
            return ControlItemActionType.WriteOnly
        if arg_str == "immediate":
            return ControlItemActionType.Immediate
        if arg_str == "delay":
            return ControlItemActionType.Delay
        if arg_str == "no-write":
    ...
    def asstr(cls, arg_val):
        if arg_val == ControlItemActionType.WriteOnly:
            return "write-only"
        if arg_val == ControlItemActionType.Immediate:
            return "immediate"
        if arg_val == ControlItemActionType.Delay:
            return "delay"
...
class ParentData(object):
    def populate(self, arg_dict):
        ...
        # Msg.trace()
        Msg.user(
            "self.work_dir: %s" % (self.work_dir), "ParentData::populate()"
        )
        ...
        self.process_max = arg_dict.get(
            CtrlItmKeys.process_max, CtrlItmDefs.process_max
        )
        self.iss_path = arg_dict.get(
            CtrlItmKeys.iss_path, CtrlItmDefs.iss_path
        )
        ...
        if not SysUtils.found(self.iss_path.find(self.test_root)):
            ...
            self.iss_path = PathUtils.include_trailing_path_delimiter(
                self.test_root
            ) + PathUtils.exclude_leading_path_delimiter(self.iss_path)
    ...
    def data(self):
        return {
            CtrlItmKeys.fdir: self.work_dir,
...
class ControlItem(object):
    ...
    def load(self, arg_item_dict, arg_parent_data):
        ...
        self.fctrl_name = arg_item_dict.get(
        ...
        # grouped options
        self.generator = arg_item_dict.get(
            CtrlItmKeys.generator, CtrlItmDefs.generator
        )
        self.iss = arg_item_dict.get(CtrlItmKeys.iss, CtrlItmDefs.iss)

        self.extract_iss_data(arg_item_dict)
        self.performance = arg_item_dict.get(
            CtrlItmKeys.performance, CtrlItmDefs.performance
        )
        self.regression = arg_item_dict.get(
    ...
    def file_path(self):
        return "%s%s%s" % (
            ...
            PathUtils.include_trailing_path_delimiter(self.fctrl_dir),
            self.fctrl_name,
        )
    ...
    def validate_item_data(self, arg_item_dict):
        # although there are many defaults there are a few things things
        # that are required
        ...
        my_keys = [
            CtrlItmKeys.options,
            CtrlItmKeys.generator,
            CtrlItmKeys.iss,
            CtrlItmKeys.performance,
            CtrlItmKeys.regression,
        ]
    ...
    # clone persistent values, the only exception is the work directory
    # which is the location of the control file from where this control item
    # data was loaded
    ...
    # this control item datsa is in the form of a dictionary, populate
    # the control item with the extracted values, where the values do not
    # exist use the
    def load_item_values(self, arg_item_dict):

        my_options = arg_item_dict.get(
            CtrlItmKeys.options, CtrlItmDefs.options
        )
        ...
        self.fctrl_dir = my_options.get(
            CtrlItmKeys.fdir,
            SysUtils.ifthen(
                self.parent_data.work_dir is None,
    ...
    def extract_iss_data(self, arg_item_dict):
        ...
        if my_iss_path is not None:
            # if the iss path was not specified then the iss path currently
            # in the parent data instance is the correct one
            Msg.dbg(
                "Iss Path: %s, Test Root: %s"
                % (my_iss_path, self.parent_data.test_root)
            )
            # check to see if path is absolute path
            if not SysUtils.found(
                my_iss_path.find(self.parent_data.test_root)
            ):
    ...
    def resolve_file_location(self):
        ...
        Msg.user(
            "Extracted File Path: %s, Control File Path: %s, Extracted "
            "File Name: %s"
            % (str(my_fctrl_dir), str(self.fctrl_dir), str(self.fctrl_name)),
            "FCTRL-DIR",
        )
        Msg.user(
            "1 - Control Directory: %s, Force Directory: %s, Test Root: %s, "
            "Work Dir: %s, Control File: %s, self.fctrl_dir: %s"
            % (
                my_fctrl_dir,
        ...
        Msg.user(
            "Control Directory: %s, Force Directory: %s, Test Root: %s, "
            "Work Dir: %s, Control File: %s"
            % (
                my_fctrl_dir,
                self.parent_data.force_path,
                self.parent_data.test_root,
        ...
        # because of the requirement of not knowing and needing to discover
        # the directory situation it is necessary to search for the control
        # file
        my_tmp = None
        my_tmp_dir = None

        # if the a directory was specified in the control name or a specifies
        # as part of the control item it is necessary to check for either a
        # absolute path or relative parent path Msg.dbg( "Checking: [%s] for
        # [%s]" % ( my_fctrl_dir, self.fctrl_name ))
        ...
        if self.check_full_path(my_fctrl_dir):
            ...
            Msg.user(
                "Using Parent Directory as Control Directory", "FILE_LOCATION"
            )
            pass
            ...
            Msg.user(
                "Using Specified Control Directory as Control Directory",
                "FILE_LOCATION",
            )
            pass
            ...
            if self.item_type() == ControlItemType.TaskItem:
                my_err_queue_item = SummaryErrorQueueItem(
                    {
                        "error": "Template Not Found at Specified Location",
                ...
                my_err_queue_item = SummaryErrorQueueItem(
                    {
                        ...
                        "message": "Control File Not Found at "
                        "Specified Location",
                        "path": self.file_path(),
                        "type": str("FileNotFoundError"),
                    }
            ...
            raise Exception(
                "File[%s] Not Found at Location Specified[%s]"
        ...
        if SysUtils.found(self.fctrl_dir.find(self.parent_data.test_root)):
            ...
            self.fctrl_dir = self.fctrl_dir.replace(
                self.parent_data.test_root, ""
            )
        ...
        Msg.user(
            "Delimited Control Directory: %s" % (self.fctrl_dir),
            "FILE_LOCATION",
        )
        return True
    ...
    def check_full_path(self, arg_fctrl_dir):
        ...
        Msg.dbg(
            "Checking for control file with real path[%s][%s]"
            % (str(arg_fctrl_dir), str(self.fctrl_name))
        )
        if arg_fctrl_dir.startswith("/"):
            # there is only one scenario where the control directory starts
            # with the path delimiter and that is the path is a real path from
            # the root directory of the system, the control file must exist
            # at that location
            ...
            raise Exception(
                "Control Item File: [%s] count not be located "
                "Not Found at [%s]" % (self.fctrl_name, arg_fctrl_dir)
            )
    ...
    def check_parent_dir(self, arg_fctrl_dir):
        ...
        Msg.user(
            ...
            % (
                str(self.parent_data.test_root),
                str(self.parent_data.work_dir),
            ),
            "CHECK-PARENT-DIR",
        )
        ...
        if arg_fctrl_dir.startswith(".."):
            ...
            my_work_dir = PathUtils.append_path(
            ...
            while my_search_dir.startswith(".."):
                ...
                # The search [updated] directory started with [..] split
                # path for both the work directory and the search directory
                # this fact that the [updated] search directory still begins
                ...
                Msg.user(
                    "File Path[%s] ... " % (my_filepath), "CHECK-PARENT-DIR"
                )
    ...
    def check_work_dir(self, arg_fctrl_dir):
        ...
        Msg.dbg(
            "Checking for control file using implict path[%s] of "
            "current control file" % (str(arg_fctrl_dir))
        )
        if self.parent_data.work_dir is not None:

            my_tmp_dir = PathUtils.append_path(
                self.parent_data.work_dir, arg_fctrl_dir
            )
            if not SysUtils.found(my_tmp_dir.find(self.parent_data.test_root)):
                # Prepend the test_root on the work directory
                my_tmp_dir = PathUtils.append_path(
                    self.parent_data.test_root, my_tmp_dir
                )
    ...
    def check_ctrl_dir(self, arg_fctrl_dir):
        ...
        if PathUtils.check_found(
            ...
            self.fctrl_dir = my_tmp_dir
            Msg.user(
                "Setting Control Directory: %s ..." % (self.fctrl_dir),
                "RELATIVE PATH",
            )
            return True
        return False
    ...
    def values(self):
        self.vals = {
            CtrlItmKeys.fname: self.fctrl_name,
        ...
        my_parent_data = ParentData().populate(
            {
                CtrlItmKeys.action: self.parent_data.action,
