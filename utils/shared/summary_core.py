...
class SummaryItem(object):
    ...
    def report(self):
        ...
        if (
            ...
            if (
                ...
                == SummaryDetail.ForceCmd
            ):
                my_msg += "ForceCommand, "
            if (
                self.detail_flags & SummaryDetail.ForceResult
                == SummaryDetail.ForceResult
            ):
                my_msg += "ForceResult, "
            if (
        ...
        Msg.info(
            "[%s], Generate Command: %s"
            % (
                SysUtils.ifthen(
                    SysUtils.success(self.force_retcode), "SUCCESS", "FAILED"
                ),
                str(self.force_cmd),
            )
        )
        Msg.info(
            "Instructions Generated - Default: %d, Secondary: %d, Total: %d"
    ...
    def load_process_info(self, arg_process_info):
        ...
        # update the user label and send iteration message to the screen if
        # user is active my_usr_lbl = Msg.set_label("user", "PROCESS-RESULT")
    ...
    def load_process_log(self):
        ...
        # here is the lowdown, if a generate line exists in the process log
        # then a Result line must also exists,

        if (self.detail_flags & SummaryDetail.ForceCmd) and not (
            self.detail_flags & SummaryDetail.ForceResult
        ):
            ...
            self.load_iss_result(
                {"iss-retcode": self.process_result[0], "iss-log": None}
            )
                ...
                try:
                    for my_line in my_flog:
                        ...
                        if SysUtils.found(my_line.find("[fail]")):
                            self.force_msg = my_line.replace(
                                "[fail]", ""
                            ).strip()
                            break
                finally:
    ...
    def commit(self):
        ...
        if self.has_simulate():
            ...
            my_tgt_name = "%s%s" % (
                self.task_path,
                SysUtils.ifthen(bool(my_sim_ret), "PASS", "FAIL"),
            )
        else:
            my_tgt_name = "%s%s" % (
                self.task_path,
                SysUtils.ifthen(bool(my_gen_ret), "PASS", "FAIL"),
            )
