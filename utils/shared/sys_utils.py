...
class SysUtils:
    ...
    def envar( arg_class, arg_key, arg_defval = None, arg_def_warn = True ):
        try:
            ...
            if arg_defval is None:
                Msg.warn( arg_key + " Not found in environmental variables, NO DEFAULT VALUE SPECIFIED!" )
            elif arg_def_warn:
                Msg.warn( arg_key + " Not found in environmental variables, using default value \"" + str( arg_defval ) + "\"" )
        return arg_defval
    ...
    def open_output( arg_class, arg_fout = None, arg_ferr = None ):
        ...
        try:
            my_fout = SysUtils.ifthen( arg_fout is None, PIPE, open( arg_fout, "w" ))
        ...
        try:
            my_ferr = SysUtils.ifthen( arg_ferr is None, PIPE, open( arg_ferr, "w" ))
    ...
    #def exec_process( arg_class, arg_cmd, arg_fout = None, arg_ferr = None, arg_timeout = None, arg_kill_all = True ):
        ...
        Msg.user( "SysUtils.exec_process( ... , arg_set_process: %s" % ( str( bool( arg_set_process is not None))), "SYS-UTILS")
        Msg.flush()
        ...
        try:
               ...
               try:
                   ...
                   try:
                       ...
                       # Trigger Shutdown of spawned processes
                           ...
                           Msg.error_trace()
                           Msg.err( "Process Did Not Execute Properly: %s" % ( my_process_cmd ))
                           # my_process.kill()
                       ...
                       my_result = my_process.communicate()
                       Msg.flush()
                       return ( my_process.returncode, None, "Process Timeout Occurred", my_start_time, my_start_time )
            ...
            return ( my_process.returncode, my_stdout, my_stderr, my_start_time, my_end_time )
        ...
        return ( SysUtils.PROCESS_UNABLE_TO_SPAWN, None, "Unable to Spawn Process ....", 0, None, None )

    @classmethod
    def exec_content( arg_class, arg_content, arg_suppress = False ):
        ...
        try:
            ...
            if not arg_suppress:
                ...
                # Msg.err( "Message"      % ( str( my_exc_val )))
            ...
            Msg.err( "Message"      % ( str( my_exc_val.message )))
            Msg.error_trace( my_exc_tb )
