...
class ProcessWorkerThread(HiThread):
    ...
    def extract_results(
        ...
        Msg.dbg(
            "Return Code %s, Command Line: |%s|, Log File: %s"
        ...
        while not self.summary.queue.enqueue(my_summary_item):
            self.HeartBeat()
            pass
    ...
    def run(self):
        ...
        my_retcode = SysUtils.exec_process(
            my_cmd, my_log, my_log, self.queue_item.ctrl_item.timeout
        )

        end = DateTime.Time()

        self.extract_results(
            my_retcode, my_cmd, my_log, self.queue_item, start, end
        )
...
class ProcessThread(HiThread):
    ...
    def run(self):
        while (not self.process_queue.fully_loaded) or (
            ...
            and (
                self.thread_count.value() > 0 or self.process_queue.size() > 0
            )
        ):
            try:
                ...
                # Launch a thread with the item. Sempahore will block if we've
                # reached max workers
...
class ProcessQueueItem(object):
    def __init__(self, arg_frun_path, arg_ctrl_item, arg_content):
        #  parent_fctrl, arg_fctrl_item, arg_item_group ):
        super().__init__()
...
class ProcessQueue(HiThreadedProducerConsumerQueue):
    def __init__(
        ...
        self.process_thread = ProcessThread(
            self, self.summary, arg_max_process_count, arg_done_signal
        )
