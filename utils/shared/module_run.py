...
class CmdLineOpts(object):
    def __init__(self, arg_switches, arg_opts):
        self.opts, self.args = getopt.getopt(
            sys.argv[1:], arg_opts, arg_switches
        )
    ...
    def option_def(self, arg_switch, arg_def_val=None, arg_short=None):
        if arg_short is not None:
            ...
            return self.get_option(
                [arg_short, "--" + arg_switch.replace("=", "")], arg_def_val
            )
        return self.get_option(
            ["--" + arg_switch.replace("=", "")], arg_def_val
        )

    # returns a list of command line arguments in the order that they occurred
    # on the command line
...
class ModuleRun(object):
    ...
    def load_message_levels(self, arg_msg_lev, arg_def_lev, arg_short_lev):
        ...
        # if a (+) or a (-) is found then the command line will be appended to
        # or demoted by
        if (my_lev_str[0] == "+") or (my_lev_str[0] == "-"):
            # use the default string to build a format string, then append the
            # passed in value
            my_fmt_str = "%s%%%s" % (arg_def_lev, "s")
