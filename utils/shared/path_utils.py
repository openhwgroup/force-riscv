...
class PathUtils(object):
    ...
    # Class method that splits a path into a file name and a directory and
    # returns both
    @classmethod
    ...
    def chdir(cls, arg_dir, arg_force=False):
        ...
        try:
            os.chdir(arg_dir)
            Msg.dbg("Success Changed Directory: %s" % (arg_dir))
        except BaseException:
            return False
    ...
    @classmethod
    def expire(cls, arg_rootdir, arg_expiredate):
    ...
    def next_subdir(cls):
        my_fname = "dir.info"
        if not PathUtils.check_file(my_fname):
            with open(my_fname, "w+") as my_file:
                my_file.write("0")
                my_file.close()
            return 0

        with open(my_fname, "r+") as my_file:
            my_ndx = my_file.read()
            my_ndx = int(my_ndx) + 1
            my_file.seek(0)
            my_file.truncate()
            my_file.write(str(my_ndx))
