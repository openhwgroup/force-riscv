...
## Prints a control dictionary out in a text form interpretable directly as a Python dictionary, unlike the default string output.
#
...
## Takes whatever _def_frun.py file exists in the working directory and extracts a control file line item from it, saving to a new file.
#
def main():
    ...
    try:
        ...
        # modify the top level control line item dictionary and render to string, removing special characters and excessive spaces.
        ...
        string_form = print_vals(control_line_item).replace('\n', '').replace('\t', ' ').replace('  ', ' ').replace('  ', ' ')
