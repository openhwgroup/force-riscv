...
# This test specifies a State with memory and GPR values. It then transitions
# to that State and uses the GPRs as base registers to load the memory values.
class MainSequence(Sequence):

    # Main entry point into the test template. FORCE will invoke the generate()
    # method to start processing the test template.
    def generate(self, **kargs):
        # Get 4 random GPR indices, excluding 0. We use the first GPR as a
        # destination register for the LD instructions and the other 3 GPRs to
        # hold addresses of the memory locations specified in the State.
    ...
    def genLoadInstructions(self, gprIndices):
        for i in range(1, 4):
            # The destination register receives the memory values 1, 2, 3 that
            # we specified as part of the State
            self.genInstruction(
                "LD##RISCV",
                {
                    "rd": gprIndices[0],
