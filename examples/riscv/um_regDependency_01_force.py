...
class MyMainSequence(Sequence):
    ...
    def generate(self, **kwargs):
        ...
        for _ in range(10):
            ...
            self.notice(">>>>>>>>  Key:  {}     Value:  {}".format(
                            "rd", target_reg_id))

            if target_reg_id == 0: continue     # targed reg id of 0 does 
            ...
            if source_reg_id == target_reg_id:
                self.notice(">>>>>>>>>>  It worked!  Target reg of the first \
                        instruction is feeding the source reg of the second \
                        instruction.")
            else:
                self.notice(">>>>>>>>>>  FAIL!  Source reg id: {}    \
                        Target reg id: {}".format(source_reg_id, target_reg_id))
