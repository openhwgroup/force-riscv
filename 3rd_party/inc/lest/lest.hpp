...
# define EXPECT_FAIL       lest_EXPECT_FAIL
...
#define lest_EXPECT_FAIL( expr, msg )	\
    do \
    { \
        try \
        { \
	  ...
	  if (text(e.what()) == msg) {	\
            if ( lest_env.pass ) \
	      lest::report( lest_env.os, lest::got{ lest_LOCATION, #expr, lest::fail_type( #msg ) }, lest_env.testing ); \
            break; \
	  } \
        } \
        catch (...) {} \
        throw lest::expected{ lest_LOCATION, #expr, lest::no_fail_type( #msg ) }; \
    } \
    while ( lest::is_false() )
...
inline text fail_type( text msg )
{
    return "through FAIL(" + msg + ") call";
}

inline text no_fail_type( text msg )
{
  return "through FAIL call with message " + msg;
}
